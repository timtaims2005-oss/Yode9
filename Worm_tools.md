...
export interface CommandDefinition {
  ...
  name: string;
  description: string;
  execute: (args: any, context: OmnixContext) => Promise<any> | any;
  undo?: (args: any, context: OmnixContext) => Promise<any> | any;
...
export class OmnixSovereign {
  ...
  private history: Array<{ action: string; timestamp: number; payload: any }> = [];
  ...
  registerComponent(id: string, type: ComponentType, ref: any, metadata: Record<string, any> = {}) {
  ...
  getSnapshot(): Record<string, Omit<ComponentEntry, 'ref'>> {
    ...
    this.liveMap.forEach((e, id) => {
      s[id] = { type: e.type, state: e.state, metadata: e.metadata, lastUpdate: e.lastUpdate };
    });
    return s;
  }
  ...
  private captureState(ref: any) {
    if (ref && typeof ref === 'object') {
      if (ref.state && typeof ref.state === 'object') return { ...ref.state };
      if (ref.props && typeof ref.props === 'object') return { ...ref.props };
      return { ...ref };
    }
    return ref;
  }

  private log(action: string, payload: any) {
...
export class OmnixAbsoluteRegistry {
  ...
  register(cmd: CommandDefinition) {
    ...
    if (cmd.metadata?.aliases && Array.isArray(cmd.metadata.aliases)) {
      cmd.metadata.aliases.forEach((a: string) => this.aliases.set(a, cmd.id));
    }
  }
  ...
  unregister(id: string) {
    ...
    if (c) {
      ...
      c.metadata?.aliases?.forEach((a: string) => this.aliases.delete(a));
      this.commands.delete(id);
    }
  }

  get(id: string) {
    return this.commands.get(id) || this.commands.get(this.aliases.get(id) || '');
  }
  ...
  async execute(id: string, args: any = {}, ctx: OmnixContext) {
    const c = this.get(id);
    if (!c) throw new Error(`[Registry] Command ${id} not found.`);
    return await c.execute(args, ctx);
  }

  async undo(id: string, args: any, ctx: OmnixContext) {
    const c = this.get(id);
...
export class OmnixMemory {
  ...
  private constructor() {
    this.initDB().catch(() => console.info('[Memory] Memory-only mode'));
  }
  ...
  private storeGetAll(store: IDBObjectStore): Promise<any[]> {
  ...
  private async saveStore(name: string, data: any) {
    if (!this.db) return;
...
// ─────────────────────────────────────────────────────────
// SYSTEM 3: OMNIX QUANTUM INTERCEPTOR
// ─────────────────────────────────────────────────────────
...
export class OmnixQuantumInterceptor {
  ...
  setSessionHistory(h: any[]) { this.sessionHistory = h; }
  setUserPreferences(p: Record<string, any>) { this.userPreferences = p; }
...
User Request: ${userPrompt}`;
  ...
  extractCommands(response: string): Array<{ id: string; args: any; reason?: string }> {
...
// ─────────────────────────────────────────────────────────
// SYSTEM 4: OMNIX INSTANT EXECUTOR
// ─────────────────────────────────────────────────────────
...
export class OmnixInstantExecutor {
  ...
  async executeSequence(commands: Array<{ id: string; args: any; reason?: string }>) {
    ...
    for (let i = 0; i < jobs.length; i++) {
      ...
      try {
        ...
        job.status = 'failed'; job.error = err?.message || 'Error';
...
export class OmnixVoiceGesture {
  ...
  private constructor() {
    if (typeof window !== 'undefined') {
      ...
      if (SR) {
        ...
        this.recognition.onresult = (e: any) => this.handle(e);
        this.recognition.onerror = (e: any) => { if (e.error !== 'no-speech') console.error('[Voice]', e.error); };
      }
    }
  }
  ...
  parseText(text: string): VoiceCommand | null {
    ...
    for (const cmd of all) {
      const names = [cmd.name.toLowerCase(), ...(cmd.metadata?.aliases || []).map((a: string) => a.toLowerCase())];
      for (const name of names) {
        if (cleaned.includes(name)) {
          return { text, confidence: 0.95, matchedCommand: cmd.id, args: this.extractArgs(cleaned, cmd.argsSchema) };
        }
      }
    }

    const kwMap: Record<string, string> = {
      'افتح': 'open', 'open': 'open', 'فتح': 'open',
      'أغلق': 'close', 'close': 'close', 'غلق': 'close',
      'شغل': 'run', 'run': 'run', 'تشغيل': 'run',
      'وقف': 'stop', 'stop': 'stop', 'إيقاف': 'stop',
      'غير': 'update', 'change': 'update', 'تغيير': 'update', 'عدل': 'update',
      'أخف': 'hide', 'hide': 'hide', 'إخفاء': 'hide',
      'أظهر': 'show', 'show': 'show', 'إظهار': 'show',
    ...
    for (const [kw, type] of Object.entries(kwMap)) {
      if (cleaned.includes(kw)) {
        const list = reg.getByType(type as any);
  ...
  private handle(e: any) {
    ...
    if (final) {
      ...
      if (p && this.onCommand) this.onCommand(p);
    }
  }

  private extractArgs(text: string, schema?: Record<string, any>) {
    const args: any = {};
...
export class OmnixSelfEvolution {
  ...
  analyzePatterns() {
    ...
    top.forEach(p => {
      ...
      if (!cmd) return;
      if (p.count > 10 && (!cmd.metadata?.aliases || cmd.metadata.aliases.length < 2)) {
        this.propose({
  ...
  approve(s: EvolutionSuggestion, executeFn?: CommandDefinition['execute']) {
    ...
    if (s.type === 'evolve' && s.basedOnPattern) {
      ...
      if (base) {
        const aliases = new Set<string>(base.metadata?.aliases || []);
      ...
      reg.register({
        ...
        execute: executeFn, argsSchema: s.proposedSchema,
...
Wrap commands in <omnix-commands> JSON </omnix-commands>.`);
  ...
  registerProjectCommand(def: CommandDefinition) {
...
export function useOmnixAbsolute() {
  ...
  useEffect(() => {
    core.setProgressCallback(p => {
      setProgress(p);
      setIsProcessing(p.current < p.total);
    });
  ...
  const runCommand = useCallback(async (id: string, args?: any) => {
  ...
  const registerComponent = useCallback((id: string, type: any, ref: any, meta?: any) => {
  ...
  return {
    ...
    progress, logs, isProcessing, isListening, toggleVoice,
    suggestions,
