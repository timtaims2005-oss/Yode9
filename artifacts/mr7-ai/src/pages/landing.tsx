...
function TypewriterText({style={}}:{style?:CSSProperties}) {
  ...
  useEffect(()=>{
    ...
    else{ if(ci>0){ const t=setTimeout(()=>{ setTxt(ph.slice(0,ci-1)); setCi(c=>c-1); },20); return()=>clearTimeout(t); } else{ setDel(false); setPi(i=>(i+1)%PHRASES.length); return undefined; } }
  },[ci,del,pi]);
  ...
  return (
    <div style={{background:"#040404",borderRadius:16,border:"1px solid rgba(0,229,255,0.15)",boxShadow:"0 0 48px rgba(0,229,255,0.07),0 24px 60px rgba(0,0,0,0.6)",overflow:"hidden"}}>
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",borderBottom:"1px solid rgba(255,255,255,0.06)",background:"rgba(255,255,255,0.02)"}}>
        ...
        <span style={{fontSize:9,fontFamily:"monospace",color:"rgba(255,255,255,0.3)",marginLeft:8,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>kaligpt v6.0 — Swarm 32x · Red Team</span>
        <span style={{marginLeft:"auto",fontSize:8,fontFamily:"monospace",color:"rgba(34,197,94,0.5)",flexShrink:0}}>● LIVE</span>
      </div>
      <div style={{padding:"12px 14px",fontFamily:"monospace",fontSize:"clamp(10px,2.5vw,12px)",lineHeight:1.9,minHeight:200,overflowX:"auto"}}>
        {lines.slice(0,vis).map((l,i)=><div key={i} style={{color:l.col,wordBreak:"break-all"}}>{l.text}</div>)}
...
function ThreatMap() {
  ...
  return <div style={{width:"100%",overflowX:"auto"}}><canvas ref={ref} style={{width:"100%",minWidth:320,height:"auto",borderRadius:12,border:"1px solid rgba(226,18,39,0.15)"}}/></div>;
}
      ...
      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,backdropFilter:"blur(24px)",borderBottom:"1px solid rgba(226,18,39,0.12)",background:"rgba(0,0,0,0.88)"}}>
        <div style={{maxWidth:1280,margin:"0 auto",padding:"0 16px",height:60,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:34,height:34,borderRadius:9,background:"linear-gradient(135deg,#e21227,#c4101f)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 22px rgba(226,18,39,0.55)"}}>
              <Shield style={{width:17,height:17,color:"#fff"}}/>
            </div>
            <div>
              <span style={{fontSize:17,fontWeight:900,letterSpacing:"-0.5px"}}><span style={{color:"#e21227"}}>Kali</span>GPT</span>
              <div style={{fontSize:8,fontFamily:"monospace",color:"rgba(255,255,255,0.25)",letterSpacing:"0.15em"}} className="hidden sm:block">v6.0 · ARSENAL MODE ULTRA</div>
            </div>
          </div>
          {/* Desktop nav links */}
          ...
          <div className="flex md:hidden" style={{alignItems:"center",gap:10}}>
            <button onClick={()=>navigate("/app")} style={{padding:"7px 16px",borderRadius:9,fontSize:12,fontWeight:700,background:"linear-gradient(135deg,#e21227,#c4101f)",color:"#fff",border:"none",cursor:"pointer",boxShadow:"0 0 16px rgba(226,18,39,0.4)"}}>
              ابدأ ←
            </button>
            <button onClick={()=>setMobileMenuOpen(v=>!v)} aria-label="القائمة"
              style={{width:38,height:38,borderRadius:9,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:5,cursor:"pointer"}}>
              <span style={{display:"block",width:18,height:1.5,background:mobileMenuOpen?"#e21227":"rgba(255,255,255,0.7)",transition:"all .25s",transform:mobileMenuOpen?"rotate(45deg) translateY(5px)":"none"}}/>
              <span style={{display:"block",width:18,height:1.5,background:"rgba(255,255,255,0.7)",transition:"all .25s",opacity:mobileMenuOpen?0:1}}/>
              <span style={{display:"block",width:18,height:1.5,background:mobileMenuOpen?"#e21227":"rgba(255,255,255,0.7)",transition:"all .25s",transform:mobileMenuOpen?"rotate(-45deg) translateY(-5px)":"none"}}/>
            </button>
          </div>
        </div>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden" style={{borderTop:"1px solid rgba(226,18,39,0.12)",background:"rgba(0,0,0,0.96)",padding:"12px 16px 16px"}}>
            {[["الميزات","features"],["الترسانة","arsenal"],["ما الجديد","changelog"],["الأسعار","pricing"],["FAQ","faq"]].map(([l,id])=>(
              <button key={id} onClick={()=>{document.getElementById(id)?.scrollIntoView({behavior:"smooth"});setMobileMenuOpen(false);}}
                style={{display:"block",width:"100%",textAlign:"right",padding:"12px 8px",fontSize:15,color:"rgba(255,255,255,0.65)",background:"none",border:"none",borderBottom:"1px solid rgba(255,255,255,0.04)",cursor:"pointer"}}>
                {l}
              </button>
            ))}
          </div>
