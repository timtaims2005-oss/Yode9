...
class SpeculativeExecution {
  ...
  private _startSpeculation(id: string, target: SpeculativeTarget): void {
    ...
    (abortRegistry as unknown as { register(k: string, v: unknown): void }).register(`speculative_${id}`, controller);
  ...
  private _cancelSpeculation(id: string): void {
    ...
    if (target?.controller && !target.result) {
      ...
      (abortRegistry as unknown as { unregister(k: string): void }).unregister(`speculative_${id}`);
