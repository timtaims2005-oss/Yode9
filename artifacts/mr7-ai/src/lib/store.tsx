...
export type Settings = {
  ...
  // ── Auto-Skills: تفعيل تلقائي للمهارات بحسب رسالة المستخدم ──────────────
...
export type ProviderName = "openai" | "anthropic" | "groq" | "gemini" | "openrouter" | "custom" | "personal" | "cloudflare";
...
export type AppState = {
  ...
  // Projects (مساحات العمل المنفصلة)
...
type Action =
  ...
  | { type: "CACHE_PROJECTS"; projects: Project[] }
  | { type: "NEW_CHAT_IN_PROJECT"; projectId: string | null };
...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case "CACHE_PROJECTS":
      return { ...state, cachedProjects: action.projects };
    case "NEW_CHAT_IN_PROJECT": {
      const id = `c-${Date.now()}`;
      const chat: Chat = { id, title: "New chat", messages: [], createdAt: Date.now(), projectId: action.projectId };
      return { ...state, chats: [chat, ...state.chats], activeChatId: id };
    }
    default:
      return state;
