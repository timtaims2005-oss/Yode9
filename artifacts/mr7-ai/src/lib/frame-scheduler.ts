...
const DEFAULT_FRAME_BUDGET_MS = 14; // leave 2-3ms for browser paint
...
class FrameScheduler {
  ...
  /** Adjust the per-frame task budget (ms). Used by perf-mode profiles. */
  setFrameBudgetMs(ms: number) {
    this.frameBudgetMs = Math.max(1, ms);
  }

  get frameBudget() { return this.frameBudgetMs; }
