...
class TurboGC {
  ...
  init(): void {
    ...
    if (typeof (memoryPressure as unknown as { onPressure?: unknown })?.onPressure === "function") {
      (memoryPressure as unknown as { onPressure(cb: () => void): void }).onPressure(() => {
        this._forceCleanup();
      });
    }
  ...
  poolGet<T>(poolKey: string, factory: () => T): T {
    ...
    (pool as unknown as PooledObject<T>[]).push(newObj);
