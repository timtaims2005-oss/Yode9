...
class RequestPrioritizer {
  ...
  /** Manually override max concurrency (e.g. perf-mode profiles). Connection-quality
   *  updates will still override this on the next metrics tick. */
  setMaxConcurrent(n: number) {
    this.maxConcurrent = Math.max(0, Math.floor(n));
    this.drain();
  }
