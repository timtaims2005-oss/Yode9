...
const lzCompress = { compress: compressToBase64, decompress: decompressFromBase64 };
    ...
    for (const [key, newVal] of Object.entries(newState)) {
      if (!(key in oldState)) {
        ...
        if (
          ...
          delta.modified[key] = { __delta: nested } as unknown as DeltaValue;
        } else {
  ...
  applyDelta(id: string, delta: Delta): DeltaObject {
    ...
    for (const [key, val] of Object.entries(delta.modified)) {
      if (val && typeof val === 'object' && '__delta' in (val as object)) {
        const nestedDelta = (val as unknown as { __delta: Delta }).__delta;
