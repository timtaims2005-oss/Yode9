...
type LIcon = React.FC<{ className?: string; style?: React.CSSProperties; size?: number }>;
const CATEGORY_ICONS: Record<string, LIcon> = {
  email: FileText,
  ip: Server,
  domain: Globe,
  darkweb: Lock,
...
export const DarkWebSearchModal: React.FC<{
  ...
  const [searchMode, setSearchMode] = useState<'unified' | 'darkweb' | 'blockchain' | 'threat' | 'breach'>('unified');
  const [breachData, setBreachData] = useState<{
    breaches: Array<{ source: string; name: string; date: string; count: string; severity: string; dataClasses: string[]; verified: boolean; domain: string }>;
    breachCount: number; totalRecords: number; riskScore: number; riskLevel: string;
    sources: Record<string, { success: boolean }>; cached?: boolean;
  } | null>(null);
  ...
  const performSearch = useCallback(async () => {
    ...
    // ── HIBP / LeakCheck breach-check mode ────────────────────────────────
    if (searchMode === 'breach') {
      try {
        const isEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(query.trim());
        const body = isEmail ? { email: query.trim() } : { domain: query.trim() };
        const res = await fetch('/api/intel/darkweb', {
          method: 'POST',
        ...
        if (json.success && json.data) {
          setBreachData(json.data);
          const bs = json.data.breaches as Array<{ severity: string }>;
          setStats({
            total: bs.length,
        ...
        console.error('[DarkWeb] breach check failed:', e);
      } finally {
        setLoading(false);
      }
      return;
        ...
        >
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {/* ── BREACH CHECK RESULTS (HIBP / LeakCheck) ─────────────────── */}
              {!loading && searchMode === 'breach' && breachData && (
                <div className="p-4 space-y-3">
                  {/* Risk summary */}
                  <div className="rounded-xl border p-4 flex items-center gap-4"
                    style={{
                      borderColor: breachData.riskScore >= 75 ? '#e21227' : breachData.riskScore >= 50 ? '#f97316' : '#fbbf24',
                      background: breachData.riskScore >= 75 ? 'rgba(226,18,39,0.08)' : 'rgba(249,115,22,0.08)'
                    }}>
                    <div className="text-center shrink-0">
                      <p className="text-3xl font-black" style={{ color: breachData.riskScore >= 75 ? '#e21227' : '#f97316' }}>{breachData.riskScore}</p>
                      <p className="text-[9px] font-mono text-gray-500 uppercase">risk score</p>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-white mb-1">{breachData.breachCount} Breach{breachData.breachCount !== 1 ? 'es' : ''} Found</p>
                      <p className="text-[11px] text-gray-400 font-mono">
                        Sources: {Object.entries(breachData.sources).filter(([,s]) => s.success).map(([k]) => k).join(' · ') || 'N/A'}
                      </p>
                      {breachData.cached && <span className="text-[9px] font-mono text-blue-400">⚡ 12H CACHE</span>}
                    </div>
                  </div>

                  {/* Stat chips */}
                  <div className="grid grid-cols-4 gap-2">
                    {([['CRITICAL','#e21227'], ['HIGH','#f97316'], ['MEDIUM','#fbbf24'], ['LOW','#22c55e']] as const).map(([sev, color]) => {
                      const count = breachData.breaches.filter(b => b.severity === sev).length;
                      return (
                        <div key={sev} className="rounded-lg text-center py-2" style={{ background: color + '15', border: `1px solid ${color}25` }}>
                          <p className="text-lg font-black" style={{ color }}>{count}</p>
                          <p className="text-[8px] font-mono" style={{ color }}>{sev}</p>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  {breachData.breachCount === 0 && (
                    <div className="py-10 text-center">
                      ...
                      <p className="text-sm font-bold text-green-400">No Breaches Found</p>
                      <p className="text-xs text-gray-600 mt-1">Not found in HIBP or LeakCheck databases</p>
                    </div>
                  )}

                  {/* Breach cards */}
                  {breachData.breaches.map((b, i) => {
                    const c = b.severity === 'CRITICAL' ? '#e21227' : b.severity === 'HIGH' ? '#f97316' : b.severity === 'MEDIUM' ? '#fbbf24' : '#22c55e';
                    return (
                      <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                        className="rounded-xl border overflow-hidden" style={{ borderColor: c + '30', background: c + '08' }}>
                        <div className="flex items-center justify-between px-4 py-2.5" style={{ borderBottom: `1px solid ${c}15` }}>
                          <div className="flex items-center gap-2">
                            <Database className="w-3 h-3" style={{ color: c }} />
                            <span className="text-sm font-bold text-white">{b.name}</span>
                            {b.verified && <span className="text-[8px] px-1.5 py-0.5 rounded-full font-bold" style={{ background: '#10b98120', color: '#10b981' }}>VERIFIED</span>}
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-[9px] font-mono px-2 py-0.5 rounded-full font-bold" style={{ background: c + '20', color: c }}>{b.severity}</span>
                            <span className="text-[10px] font-mono text-gray-500">{b.count} records</span>
                          </div>
                        </div>
                        <div className="px-4 py-2.5">
                          <div className="flex items-center gap-3 flex-wrap mb-1.5">
                            {b.domain && <span className="text-[10px] font-mono text-gray-500">{b.domain}</span>}
                            <span className="text-[10px] font-mono text-gray-600">{b.date?.slice(0, 10)}</span>
                            <span className="text-[10px] font-mono text-gray-600">via {b.source}</span>
                          </div>
                          <div className="flex flex-wrap gap-1">
                            {b.dataClasses.slice(0, 6).map((cls) => (
                              <span key={cls} className="text-[9px] px-1.5 py-0.5 rounded bg-gray-800 text-gray-400 font-mono">{cls}</span>
                            ))}
                          </div>
                        </div>
                      </motion.div>
                  ...
                  <div className="rounded-xl border border-gray-800 bg-gray-900/30 overflow-hidden">
                    <div className="px-4 py-2 border-b border-gray-800">
                      <p className="text-[9px] font-mono uppercase tracking-widest text-gray-600">Sources Queried</p>
                    </div>
                    {Object.entries(breachData.sources).map(([src, s]) => (
                      <div key={src} className="flex items-center justify-between px-4 py-2 border-b border-gray-800/50 last:border-0">
                        <span className="text-[11px] font-mono text-gray-400">{src}</span>
                        <span className={`text-[10px] font-mono ${s.success ? 'text-green-400' : 'text-gray-600'}`}>
                          {s.success ? '✓ OK' : '— unavailable'}
                        </span>
                      </div>
                    ))}
                  </div>
                          ...
                          {isExpanded && (
                            ...
                            >
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                {/* Monero Addresses */}
                                {(result.moneroAddresses?.length ?? 0) > 0 && (
                                ...
                                {/* Technologies */}
                                {(result.technologies?.length ?? 0) > 0 && (
