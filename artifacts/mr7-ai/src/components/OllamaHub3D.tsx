...
export function OllamaHub3D({ open, onClose }: OllamaHubProps) {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const r  = await localFetch("/api/ollama/status");
      const d  = await r.json() as OllamaStatus;
      setStatus(d);
      if (d.running) {
        const ps = await localFetch("/api/ollama/ps");
  ...
  const fetchDlStatus = useCallback(async () => {
    try {
      const r = await localFetch("/api/ollama/dl-status");
  ...
  const fetchSysInfo = useCallback(async () => {
    try {
      const r = await localFetch("/api/ollama/sysinfo");
  ...
  const handleScanCleanup = useCallback(async () => {
    ...
    try {
      const r = await localFetch("/api/ollama/cleanup-blobs");
  ...
  const handleDoCleanup = useCallback(async () => {
    setCleanupLoading(true);
    try {
      await localFetch("/api/ollama/cleanup-blobs", { method: "DELETE" });
      setCleanupFiles([]);
      setCleanupDone(true);
      await fetchSysInfo();
    } catch { /* skip */ }
  ...
  const handleTestConnection = useCallback(async () => {
    ...
    try {
      const r = await localFetch("/api/ollama/test-connection");
  ...
  const handleBenchmark = useCallback(async () => {
    ...
    for (const modelName of modelsCopy) {
      ...
      for (let i = 0; i < benchRuns; i++) {
        ...
        let firstToken = true;
        try {
          const r = await localFetch("/api/ollama/chat/stream", {
            method: "POST",
  ...
  const handleInstall = async () => {
    ...
    setTab("dashboard");
    try {
      const r = await localFetch("/api/ollama/install", { method: "POST" });
      if (!r.body) return;
      const reader  = r.body.getReader();
  ...
  const handlePull = async (modelName: string) => {
    ...
    try {
      const r = await localFetch("/api/ollama/pull", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (name: string) => {
    await localFetch("/api/ollama/delete", {
  ...
  const handleChat = async () => {
    ...
    try {
      ...
      try {
        const r2 = await localFetch("/api/ollama/chat", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ model: chatModel, messages: newHistory }),
        });
