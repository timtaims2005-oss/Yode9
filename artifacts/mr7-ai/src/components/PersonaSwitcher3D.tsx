...
function PersonaOrb({ color, pulse }: { color: [number, number, number]; pulse: boolean }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      const halo = ctx.createRadialGradient(cx, cy, 6, cx, cy, SIZE / 2);
      ...
      ctx.rotate(-t * 0.4);
      ctx.beginPath();
      ctx.ellipse(0, 0, 10, 3.5, Math.PI / 4, 0, Math.PI * 2);
      ...
      // orbiting particle
      const pa = t * 1.4;
      const px = cx + Math.cos(pa) * 13;
      const py = cy + Math.sin(pa) * 4;
      const pGlow = ctx.createRadialGradient(px, py, 0, px, py, 3);
  ...
  return (
    <canvas
      ref={canvasRef}
      style={{ width: 36, height: 36, imageRendering: "crisp-edges", flexShrink: 0 }}
    />
  );
}
  ...
  return (
    <div ref={ref} className="relative flex-shrink-0">
      ...
      >
        ...
        <PersonaOrb color={color} pulse={isCustomActive} />
        ...
        {showPanel && (
          ...
          >
            ...
            <div className="mx-3 mb-2 rounded-xl p-2.5 flex items-center gap-2.5"
              ...
              <PersonaOrb color={color} pulse={true} />
