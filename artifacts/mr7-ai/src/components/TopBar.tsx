...
function useLocalModelHealth(endpoint: string, enabled: boolean) {
  ...
  const ping = useCallback(async () => {
    ...
    const t0 = Date.now();
    try {
      const res = await localFetch("/api/local-engines/status", { signal: AbortSignal.timeout(6000) });
...
function OperationModeBtn3D() {
  ...
  const savePerfMode  = (m: PerfMode)     => { setPerfMode(m);  localStorage.setItem(LS_PERF, m);  window.dispatchEvent(new Event("mr7-op-change")); };
  const saveWflowMode = (m: WorkflowMode) => { setWflowMode(m); localStorage.setItem(LS_WFLOW, m); window.dispatchEvent(new Event("mr7-op-change")); };
