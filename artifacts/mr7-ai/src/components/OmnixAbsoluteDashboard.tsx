...
export function OmnixAbsoluteDashboard({ open, onClose }: OmnixAbsoluteDashboardProps) {
  ...
  const [patterns, setPatterns] = useState<{ commandId: string; label: string; count: number }[]>([]);
  ...
  useEffect(() => {
    ...
    setPatterns(OmnixMemory.getInstance().getTopPatterns(10).map(p => ({ commandId: p.commandId, label: p.commandId, count: p.count })));
...
function OverviewTab({ status, patterns, logs, suggestions, core }: {
  ...
  patterns: { commandId: string; label: string; count: number }[];
  logs: ExecutionJob[];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-[#111] border border-[#1f1f1f] rounded-xl p-4">
          ...
          <div className="space-y-2">
            ...
            {patterns.slice(0, 6).map((p: { commandId: string; label: string; count: number }) => (
