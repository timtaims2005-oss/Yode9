...
const ADDITIONAL_TOOLS: { icon: React.ElementType; label: string; color?: string }[] = [
...
export function Sidebar({ isOpen, onClose, collapsed, onToggleCollapsed, onOpenPricing, onOpenApi, onOpenTool, onOpenSettings, onOpenAccount, onOpenLogin, onOpenUtility, onOpenToolsHub, onOpenMemory, onOpenBookmarks, onOpenSearch, onOpenCompare, onOpenQRSync, onOpenChangelog, onOpenOsint, onOpenOsintHub, onOpenUseCaseLib, onOpenOmegaAgent, onOpenLocalEngineHub, onOpenMultiModelRace, onOpenLocalBenchmark, onOpenKgAdmin, onOpenKgPayment, onOpenKgRAG, onOpenKgMemory, onOpenKgNotifications, onOpenKgMultiAgent, onOpenKgOrganizations, onOpenKgMarketplace, onOpenKgAnalytics, onOpenKgFinetune, onOpenKgAPIKeys, onOpenKgMonitoring, onOpenAdvancedDashboard, onOpenKgSemanticSearch, onOpenKgCollaboration, onOpenKgContext, onOpenKgPentestLab, onOpenKgSecurity, onOpenKgHelpCenter, onOpenKgReports, onOpenKgRateLimit, onOpenKgSystemsHub, onOpenKgSwarmEvolution, onOpenKgProjectGenerator, onOpenKgAgentMemory, onOpenKgAutonomousSwarm, onOpenKgSocialArsenal, onOpenAptIntel, onOpenNexusPanel, onOpenAccountHackeTools, onOpenOmnixAbsolute, onOpenDarkWebIntelFull, onOpenCodeInterpreter, onOpenCustomGpts, onOpenScheduledTasks, onOpenOrchestratorHub, onOpenEcosystem }: SidebarProps) {
  ...
  // Fix: وسّعنا العرض الأدنى لـ 300px وقيّدناه بـ 90vw بدلاً من 100vw
  // حتى تتسع أزرار Telegram/Instagram/WhatsApp على الشاشات الصغيرة (أقل من 360px)
  const content = (
    <div className="CHAT-GPT-sidebar flex flex-col h-full w-[min(300px,90vw)] text-sm overflow-hidden flex-shrink-0 relative"
      ...
      {/* Fix: أضفنا خصائص التمرير للموبايل — WebkitOverflowScrolling لـ iOS momentum scrolling,
          overscrollBehavior لمنع chain scroll للـ body, touchAction لضمان pan-y gesture */}
      ...
      >
        ...
        {/* Fix: دمجنا محتوى Footer داخل منطقة التمرير بدلاً من تركه div ثابت خارجها.
            السبب: الحاوية الجذرية عندها overflow-hidden، فأي محتوى يتجاوز الشاشة في الـ Footer
            الثابت يُقتَص تلقائياً بدون إمكانية وصول — المحتوى ضائع بصرياً. الحل: نجعله
            جزءاً من نفس div القابل للتمرير لضمان الوصول إليه بالسحب. */}
        ...
        {(onOpenKgAdmin || onOpenKgAnalytics || onOpenKgPentestLab || onOpenKgSecurity || onOpenKgSystemsHub) && (
          <div className="mb-2">
            ...
            <div className="grid grid-cols-3 gap-1">
              ...
              {onOpenAdvancedDashboard && (
                <button onClick={onOpenAdvancedDashboard} title="🚀 Unified System v2.0 — Enterprise Dashboard" className="flex flex-col items-center gap-0.5 w-7 h-7 flex items-center justify-center rounded-lg bg-[#e21227]/15 hover:bg-[#e21227]/30 border border-[#e21227]/30 hover:border-[#e21227]/60 transition-all group relative">
                  <LayoutGrid className="w-3.5 h-3.5 text-[#e21227]/80 group-hover:text-[#e21227]" />
                  <span className="text-[8px] text-gray-500 group-hover:text-gray-300">Ultra</span>
                  <span className="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-[#e21227] animate-pulse" />
                </button>
              )}
              {onOpenKgAPIKeys && (
              ...
              {onOpenEcosystem && (
                <button onClick={onOpenEcosystem} title="مركز التحكم بالمنظومة المستقلة — Ecosystem Control Center" className="flex h-7 w-7 flex flex-col items-center justify-center gap-0.5 rounded-lg border border-cyan-500/25 bg-cyan-950/30 transition-all hover:border-cyan-400/70 hover:bg-cyan-900/40 group relative">
                  <div className="absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full border border-black bg-cyan-300 animate-pulse" />
                  <Network className="h-3.5 w-3.5 text-cyan-300/80 group-hover:text-cyan-200" />
                  <span className="text-[8px] text-gray-400 group-hover:text-white">ECO</span>
                </button>
              )}
              {onOpenKgSecurity && (
        ...
        {/* Main Agent Control — compact toggle */}
        <Suspense fallback={null}>
          <MainAgentControl compact className="mb-2" />
        </Suspense>
        ...
        </div>{/* ← يُغلق footer inner div (p-3 border-t ...) المدموج في منطقة التمرير */}
      </div>{/* ← يُغلق flex-1 overflow-y-auto — الحاوية الرئيسية الموحّدة للتمرير */}
    </div>
  );
