...
const ALL_PROVIDERS: ProviderDef[] = [
  ...
  {
    id: "cloudflare", name: "Cloudflare Workers AI", shortName: "CF", color: "#f6821f",
    baseURL: "https://api.cloudflare.com/client/v4/accounts/{account_id}/ai/run", providerName: "cloudflare",
    category: "مجاني دائم", requiresKey: false, badge: "مجاني دائم",
    models: [
      { id: "@cf/meta/llama-3.1-8b-instruct", label: "Llama 3.1 8B", tag: "BEST" },
      { id: "@cf/meta/llama-3.2-3b-instruct", label: "Llama 3.2 3B", tag: "FAST" },
      { id: "@cf/mistral/mistral-7b-instruct-v0.1", label: "Mistral 7B", tag: "OPEN" },
      { id: "@cf/google/gemma-7b-it", label: "Gemma 7B", tag: "OPEN" },
      { id: "@cf/qwen/qwen1.5-7b-chat-awq", label: "Qwen 1.5 7B", tag: "OPEN" },
    ],
  },
  {
...
export function AIQuickSetupButton() {
  ...
  useEffect(() => {
    ...
    ALL_PROVIDERS.forEach((p) => {          // intentional: runs once on open, before dynamic data
  ...
  const autoScan = useCallback(async () => {
    ...
    for (let i = 0; i < allProviders.length; i++) {
      ...
      setScanProgress(Math.round((i / allProviders.length) * 100));
  ...
  // ── Dynamic Cloudflare models from API ─────────────────────────────────────
  const { data: cfApiModels = [] } = useQuery<Array<{ name: string; description?: string }>>({
    queryKey: ["cf-models-quick-setup"],
    queryFn: async () => {
      const r = await fetch("/api/providers/cloudflare/models");
      if (!r.ok) return [];
      const d = await r.json() as { ok: boolean; models: Array<{ name: string; description?: string }> };
      return d.ok ? d.models : [];
    },
  ...
  // Merge static list: replace CF static models with live API models
  const allProviders = ALL_PROVIDERS.map((p) => {
    if (p.id !== "cloudflare" || cfApiModels.length === 0) return p;
    return {
      ...p,
      category: `${cfApiModels.length} نموذج مجاني`,
      models: cfApiModels.map((m) => {
        const txt = (m.name ?? "").toLowerCase();
        const match = txt.match(/(\d+(?:\.\d+)?)b/);
        const params = match ? parseFloat(match[1]) : null;
        const tag = params === null ? "CF" : params < 4 ? "FAST" : params > 20 ? "POWER" : "OPEN";
        return {
          id: m.name,
  ...
  const cfgCnt = allProviders.filter((p) => (p.requiresKey ? (keys[p.id]?.length ?? 0) > 10 : true)).length;
  const filteredProviders = allProviders.filter((p) => !providerSearch || p.name.toLowerCase().includes(providerSearch.toLowerCase()));
            ...
            >
              <div>
                ...
                <div className="text-[8px] mt-0.5" style={{ color: "rgba(255,255,255,0.32)" }}>
                  {cfgCnt} مزوّد مُهيَّأ من {allProviders.length}
                </div>
              </div>
              <div className="flex items-center gap-1.5">
                <button
