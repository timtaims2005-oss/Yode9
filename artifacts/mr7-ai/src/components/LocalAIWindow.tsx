...
export function LocalAIWindow({
  ...
  const fetchStatus = useCallback(async () => {
    setRefreshing(true);
    try {
      const r = await localFetch("/api/local-engines/status");
      if (r.ok) {
  ...
  useEffect(() => {
    ...
    const checkPhi3 = async () => {
      try {
        const r = await localFetch("/api/local-engines/phi3-status");
        if (r.ok) {
  ...
  const installEngine = async (id: string) => {
    ...
    try {
      const r = await localFetch(`/api/local-engines/install/${id}`, { method: "POST" });
      if (r.body) {
  ...
  const launchEngine = async (id: string) => {
    ...
    try {
      const r = await localFetch(`/api/local-engines/launch/${id}`, { method: "POST" });
      if (r.body) {
        const reader = r.body.getReader();
        const dec    = new TextDecoder();
        let   carry  = "";
  ...
  const downloadPhi3 = async () => {
    ...
    setPhi3Pct(0);
    try {
      const r = await localFetch("/api/local-engines/llamafile-model", { method: "POST" });
      if (r.body) {
        const reader = r.body.getReader();
        const dec    = new TextDecoder();
        let   carry  = "";
  ...
  const deleteOllamaModel = async (model: string) => {
    ...
    setDeleteError(null);
    try {
      const r = await localFetch(`/api/local-engines/model/${encodeURIComponent(model)}`, { method: "DELETE" });
  ...
  const runBenchmark = async (model?: string) => {
    ...
    setBenchTokens(null);
    try {
      const r = await localFetch("/api/local-engines/benchmark", {
        method: "POST",
  ...
  const pullOllamaModel = async (overrideModel?: string) => {
    ...
    setPullPct(null);
    try {
      const r = await localFetch("/api/local-engines/pull-model", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const streamDuelSide = async (
    ...
    let tokens = 0;
    try {
      const resp = await localFetch("/api/local-proxy/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        signal: ac.signal,
        body: JSON.stringify({
