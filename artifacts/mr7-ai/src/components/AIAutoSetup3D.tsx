...
export function detectAllConfiguredProviders(): DP[] {
  return PROVIDER_PRIORITY.flatMap((p): DP[] => {
    if (p.requiresKey) {
      const k = localStorage.getItem(KP+p.id)?.trim() ?? "";
      if (k.length > 10) return [{ ...p, key: k, url: localStorage.getItem(UP+p.id)?.trim() || p.baseURL } as DP];
      return [];
    }
    return [{ ...p, key: "", url: p.baseURL } as DP];
  });
}
...
export function AIAutoSetup3D({onComplete}:{onComplete:()=>void}) {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      if(persKey&&persKey.length>10&&!found.find(f=>(f.id as string)==="personal"))
        found.unshift({...PROVIDER_PRIORITY[5],id:"personal",name:"Personal API",color:"#e21227",category:"شخصي",key:persKey,url:state.settings?.personalApiBaseURL||"https://api.openai.com/v1"} as unknown as DP);
