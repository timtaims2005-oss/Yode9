      ...
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
            className="w-full rounded-[18px] border overflow-hidden flex flex-col relative"
            style={{ width: "clamp(340px, 42vw, 580px)", backdropFilter: "blur(40px)", background: "linear-gradient(160deg, rgba(9,5,18,0.97) 0%, rgba(5,3,12,0.98) 100%)", borderColor: "rgba(139,92,246,0.28)", maxHeight: "90vh", boxShadow: "0 0 60px rgba(139,92,246,0.1), 0 32px 64px rgba(0,0,0,0.6), inset 0 1px 0 rgba(139,92,246,0.12)" }}>

            {/* Top accent line */}
            <div className="absolute inset-x-0 top-0 h-px z-10" style={{ background: "linear-gradient(90deg, transparent, rgba(139,92,246,0.9), rgba(167,139,250,0.6), transparent)" }} />

            <div className="flex items-center justify-between px-4 pt-3 pb-[10px] border-b" style={{ borderColor: "rgba(139,92,246,0.12)", background: "rgba(0,0,0,0.35)" }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[10px] font-mono" style={{ color: "rgba(255,255,255,0.42)" }}>Hack the AI Agent · Gemini API Security</div>
                </div>
              </div>
            ...
            {/* Tabs */}
            <div className="flex border-b px-5" style={{ borderColor: "rgba(255,255,255,0.08)" }}>
              {(["skills", "book"] as const).map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  ...
                  style={{ borderColor: activeTab === tab ? "#8b5cf6" : "transparent", color: activeTab === tab ? "#8b5cf6" : "rgba(255,255,255,0.42)" }}>
            ...
            {activeTab === "skills" && (
              <div className="flex flex-1 min-h-0">
                <div className="w-48 border-r py-2" style={{ borderColor: "rgba(255,255,255,0.07)", background: "rgba(0,0,0,0.45)" }}>
                  {SKILL_CATEGORIES.map(c => {
                    ...
                    return (
                      <button key={c.id} onClick={() => { setSelectedCat(c.id); setSelectedSkill(""); }}
                        ...
                        style={{ background: selectedCat === c.id ? `${c.color}18` : "transparent", color: selectedCat === c.id ? c.color : "rgba(255,255,255,0.45)", border: `1px solid ${selectedCat === c.id ? `${c.color}30` : "transparent"}`, width: "calc(100% - 16px)" }}>
                ...
                <div className="flex-1 flex flex-col min-w-0 p-5">
                  ...
                  <p className="text-[10px] font-mono mb-3" style={{ color: "rgba(255,255,255,0.48)" }}>{cat.desc}</p>

                  <div className="text-[9px] font-bold uppercase tracking-wider mb-2" style={{ color: "rgba(255,255,255,0.38)" }}>Techniques</div>
                  ...
                  <div className="text-[9px] font-bold uppercase tracking-wider mb-2" style={{ color: "rgba(255,255,255,0.38)" }}>
                    AI Research{selectedSkill ? ` · ${selectedSkill}` : ""}
                  </div>
                  {response && (
                    <div className="overflow-y-auto rounded-xl p-3 mb-3 text-[11px] font-mono leading-relaxed" style={{ background: "rgba(0,0,0,0.5)", border: `1px solid ${cat.color}18`, color: "rgba(255,255,255,0.7)", whiteSpace: "pre-wrap", maxHeight: "200px" }}>
                      {response}
                  ...
                  <div className="flex gap-2 mt-auto">
                    <input value={query} onChange={e => setQuery(e.target.value)}
                      ...
                      className="flex-1 px-3 py-2 rounded-xl text-[11px] font-mono border outline-none transition-all"
                      style={{ background: "rgba(0,0,0,0.5)", borderColor: `${cat.color}30`, color: "rgba(255,255,255,0.82)" }}
                      onFocus={e => (e.currentTarget.style.borderColor = `${cat.color}60`)}
                      onBlur={e => (e.currentTarget.style.borderColor = `${cat.color}30`)} />
            ...
            {activeTab === "book" && (
              <div className="flex-1 overflow-y-auto p-5">
                <div className="mb-4 p-4 rounded-xl relative overflow-hidden" style={{ borderColor: "rgba(139,92,246,0.3)", border: "1px solid rgba(139,92,246,0.3)", background: "rgba(139,92,246,0.08)" }}>
                  <div className="absolute inset-x-0 top-0 h-px" style={{ background: "linear-gradient(90deg, transparent, rgba(139,92,246,0.7), transparent)" }} />
                  <div className="text-[11px] font-bold mb-1" style={{ color: "#a78bfa" }}>Hack the AI Agent</div>
                  <div className="text-[10px] font-mono" style={{ color: "rgba(255,255,255,0.42)" }}>Build agentic AI security skills with the Gemini API · Google · 2025</div>
                </div>
                <div className="space-y-2">
                  {BOOK_CHAPTERS.map((ch, i) => (
                    <button key={i} onClick={() => { setQuery(`Explain the content and key concepts from "${ch}" in the "Hack the AI Agent" book`); setActiveTab("skills"); }}
                      className="w-full text-left p-3.5 rounded-xl border transition-all hover:border-purple-500/40 group"
                      style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.03)" }}>
                      <div className="flex items-center gap-3">
                        <BookOpen className="w-3.5 h-3.5 flex-shrink-0" style={{ color: "rgba(139,92,246,0.7)" }} />
                        <span className="text-[11px] font-mono" style={{ color: "rgba(255,255,255,0.55)" }}>{ch}</span>
                        <Zap className="w-3 h-3 ml-auto opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: "#a78bfa" }} />
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <div className="px-4 pt-3 pb-[10px] border-t flex items-center justify-between" style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(0,0,0,0.4)" }}>
              <div className="text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.28)" }}>{SKILL_CATEGORIES.length} categories · Defensive research · CTF</div>
              <Brain className="w-3.5 h-3.5" style={{ color: "rgba(139,92,246,0.5)" }} />
            </div>
          </motion.div>
        </motion.div>
      )}
