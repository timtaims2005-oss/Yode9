...
export function ChainInvestigationModal({ open, onOpenChange }: Props) {
  ...
  // ── threat intel enrichment (structured endpoint) ─────────────────────────
  const enrichNode = useCallback(async (nodeId: string) => {
    ...
    try {
      const res = await fetch("/api/threat-intel/enrich", {
        ...
        body: JSON.stringify({ nodeType: node.type, value: node.value || node.label }),
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const json = await res.json() as { success: boolean; data?: ThreatIntel; error?: string };
      if (!json.success || !json.data) throw new Error(json.error ?? "فشل");
      ...
      const msg = e instanceof Error ? e.message : "فشل جلب بيانات الاستخبارات";
      setNodes((p) => p.map((n) => n.id === nodeId ? { ...n, intel: { loading: false, summary: msg } } : n));
      toast({ description: "فشل التحليل الاستخباراتي", variant: "destructive" });
    }
    setIsEnriching(false);
  }, [nodes, toast]);

  // ── full-chain AI analysis (structured endpoint) ──────────────────────────
  const runAnalysis = useCallback(async () => {
    if (nodes.length < 2) return;
    setIsAnalyzing(true); setAnalysis(null);
    try {
      const res = await fetch("/api/intel/chain", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          nodes: nodes.map((n) => ({
            id: n.id, type: n.type, label: n.label,
            value: n.value || undefined, risk: n.risk ?? "low",
          })),
          links: links.map((l) => ({
            from: l.from, to: l.to, relation: l.relation, strength: l.strength,
          })),
        }),
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const json = await res.json() as {
        ...
        data?: {
          ...
          patterns: { title: string; description: string; severity: string }[];
          attackPath: string;
          recommendations: string[];
          ttps: { id: string; name: string; tactic: string }[];
          adjacency?: Record<string, string[]>;
          nodes?: Array<{ id: string; risk: string; intel?: { summary?: string } }>;
        };
        error?: string;
      };
      if (!json.success || !json.data) throw new Error(json.error ?? "فشل التحليل");
      const d = json.data;
      ...
      const parts: string[] = [
        `## ملخص الشبكة\n${d.summary}`,
        d.patterns.length > 0
          ? `\n## الأنماط الخطيرة المكتشفة\n${d.patterns.map((p) => `**[${p.severity}] ${p.title}:** ${p.description}`).join("\n")}`
          : "",
        d.attackPath ? `\n## مسار الهجوم المحتمل\n${d.attackPath}` : "",
        d.recommendations.length > 0
          ...
          : "",
        d.ttps.length > 0
          ? `\n## MITRE ATT&CK TTPs\n${d.ttps.map((t) => `- **${t.id}** ${t.name} (${t.tactic})`).join("\n")}`
          : "",
      ].filter(Boolean);

      setAnalysis({ summary: parts.join("\n"), riskScore: d.riskScore });

      // Merge AI-enriched node risk + intel summary back into the graph
      if (Array.isArray(d.nodes) && d.nodes.length > 0) {
        const enrichedMap = new Map(d.nodes.map((n) => [n.id, n]));
        setNodes((prev) =>
          prev.map((n) => {
            const enriched = enrichedMap.get(n.id);
            if (!enriched) return n;
            return {
              ...n,
              risk: (enriched.risk as typeof n.risk) ?? n.risk,
              intel: enriched.intel?.summary
                ? { ...(n.intel ?? {}), summary: enriched.intel.summary }
                : n.intel,
            };
          }),
        );
      ...
      toast({ description: e instanceof Error ? e.message : "فشل التحليل", variant: "destructive" });
    }
