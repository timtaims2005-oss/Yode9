...
export type ArsenalModuleId =
  ...
  | "mobilesecurity"
  // Batch 16 — Uploaded Arsenal (Aider · Open Design · Open Codesign · AI Website Cloner · Strix · Awesome Hacking · Open Design Pro)
  | "aider" | "opendesign" | "opencodesign" | "aiwebsitecloner" | "strix" | "awesomehacking" | "opendesignpro";
...
export const ARSENAL_MODULES: ArsenalModule[] = [
  ...
  // ── Batch 16 — Uploaded Arsenal (7 GitHub ZIPs) ───────────────────────────
  {
    id: "aider", name: "Aider", subtitle: "AI Pair Programming · Terminal · 100+ Languages · 88% Singularity",
    desc: "AI pair programming in your terminal. Works with Claude Opus, GPT-4o, DeepSeek R1, Gemini, and local LLMs. Maps your entire codebase, supports 100+ languages, auto-commits changes, runs tests, and writes 88% of its own code.",
    icon: Code2, color: "#f97316", border: "rgba(249,115,22,0.4)", bg: "rgba(249,115,22,0.07)", glow: "rgba(249,115,22,0.25)",
    source: "Aider-AI/aider", tag: "AI CODING",
  },
  {
    id: "opendesign", name: "Open Design", subtitle: "Open-Source Design Tool · Figma Alternative · Real-Time Collab",
    desc: "Open-source, collaborative design tool. Vector editing, components, design tokens, real-time collaboration, multi-framework code export, and AI-powered design assistant. Full Figma importer support.",
    icon: Palette, color: "#3b82f6", border: "rgba(59,130,246,0.4)", bg: "rgba(59,130,246,0.07)", glow: "rgba(59,130,246,0.25)",
    source: "opendesigndev/open-design", tag: "DESIGN",
  },
  {
    id: "opencodesign", name: "Open Codesign", subtitle: "Code + Design Platform · Live Preview · Team Collab · CSS Editor",
    desc: "Unified code and design platform where engineers and designers collaborate in real-time. Live CSS editor with instant preview, team chat, responsive breakpoints, git integration, and AI-powered code generation.",
    icon: Layers, color: "#14b8a6", border: "rgba(20,184,166,0.4)", bg: "rgba(20,184,166,0.07)", glow: "rgba(20,184,166,0.25)",
    source: "open-codesign/open-codesign", tag: "COLLAB",
  },
  {
    id: "aiwebsitecloner", name: "AI Website Cloner", subtitle: "Clone Any Website → React/Vue/Svelte · Design Tokens · AI-Optimized",
    desc: "Clone any website into production-ready code using multimodal AI. Analyzes URL structure, extracts design tokens, downloads assets, generates React/Vue/Svelte components with Tailwind, and AI-optimizes every component.",
    icon: Globe, color: "#06b6d4", border: "rgba(6,182,212,0.4)", bg: "rgba(6,182,212,0.07)", glow: "rgba(6,182,212,0.25)",
    source: "ai-website-cloner-template", tag: "CLONER",
  },
  {
    id: "strix", name: "Strix", subtitle: "Autonomous AI Pentesting · Multi-Agent · PoC Validation · Auto-Fix",
    desc: "Open-source AI pentesting agents that act like real hackers — run code dynamically, find vulnerabilities, and generate validated PoC exploits. 5 autonomous agents: Recon, Spider, Exploit, Validator, Report. Auto-fix patches included.",
    icon: Shield, color: "#4ade80", border: "rgba(74,222,128,0.4)", bg: "rgba(74,222,128,0.07)", glow: "rgba(74,222,128,0.25)",
    source: "usestrix/strix", tag: "AI PENTEST",
  },
  {
    id: "awesomehacking", name: "Awesome Hacking", subtitle: "Curated Security Resources · 54+ Repos · OSINT · Pentest · Forensics",
    desc: "Curated collection of awesome hacking repositories and security resources. 54+ categories including Android Security, AppSec, Bug Bounty, CTF, Forensics, Malware Analysis, OSINT, Penetration Testing, Reverse Engineering, and Web Security.",
    icon: BookOpen, color: "#a78bfa", border: "rgba(167,139,250,0.4)", bg: "rgba(167,139,250,0.07)", glow: "rgba(167,139,250,0.25)",
    source: "Hack-with-Github/Awesome-Hacking", tag: "DIRECTORY",
  },
  {
    id: "opendesignpro", name: "Open Design Pro", subtitle: "v0.15.1 Stable · 248 Components · 154 Tokens · 40+ Plugins",
    desc: "Stable release of Open Design with the full component library: 248 components, 154 design tokens, 40+ plugins including AI Copilot, Figma Bridge, Storybook Sync, and Lottie Animator. WCAG 2.2 accessibility linting built-in.",
    icon: Sparkles, color: "#6366f1", border: "rgba(99,102,241,0.4)", bg: "rgba(99,102,241,0.07)", glow: "rgba(99,102,241,0.25)",
    source: "opendesigndev/open-design v0.15.1", tag: "DESIGN PRO",
  },
];
...
const ARSENAL_CSS = `
  ...
  @keyframes arsenal-title-glow {
    0%, 100% { text-shadow: 0 0 12px #e21227, 0 0 24px rgba(226,18,39,0.45); }
    50% { text-shadow: 0 0 24px #e21227, 0 0 48px rgba(226,18,39,0.75), 0 0 72px rgba(226,18,39,0.3); }
  }
  @keyframes arsenal-spin-slow {
    from { transform: rotate(0deg); }
    to { transform: rotate(360deg); }
  }
  @keyframes arsenal-gradient-shift {
    0%, 100% { background-position: 0% 50%; }
    50% { background-position: 100% 50%; }
  }
  @keyframes arsenal-stat-enter {
    from { opacity: 0; transform: translateY(6px) scale(0.97); }
    to   { opacity: 1; transform: translateY(0)  scale(1); }
  }
  @keyframes arsenal-tab-slide {
  ...
  .arsenal-card-shimmer::before {
    ...
    position: absolute;
    inset: 0;
    background: linear-gradient(105deg, transparent 20%, rgba(255,255,255,0.035) 40%, rgba(255,255,255,0.06) 50%, rgba(255,255,255,0.035) 60%, transparent 80%);
    animation: arsenal-shimmer 5s ease-in-out infinite;
    pointer-events: none;
  ...
  .arsenal-scan-line {
    ...
    background: linear-gradient(90deg, transparent 0%, rgba(226,18,39,0.0) 20%, rgba(226,18,39,0.6) 50%, rgba(226,18,39,0.0) 80%, transparent 100%);
    animation: arsenal-scan 8s linear infinite;
    pointer-events: none;
    z-index: 1;
    box-shadow: 0 0 16px rgba(226,18,39,0.35);
  }
  .arsenal-card:hover .arsenal-card-launch {
    opacity: 1;
  ...
  .arsenal-stat-card {
    animation: arsenal-stat-enter 0.35s ease both;
  }
  .arsenal-glass {
  ...
  .arsenal-input:focus {
    outline: none;
    border-color: rgba(226,18,39,0.5) !important;
    box-shadow: 0 0 0 3px rgba(226,18,39,0.08), 0 0 24px rgba(226,18,39,0.1) !important;
  }
  .arsenal-scrollbar::-webkit-scrollbar { width: 4px; }
  .arsenal-scrollbar::-webkit-scrollbar-track { background: transparent; }
  .arsenal-scrollbar::-webkit-scrollbar-thumb { background: rgba(226,18,39,0.2); border-radius: 2px; }
  .arsenal-scrollbar::-webkit-scrollbar-thumb:hover { background: rgba(226,18,39,0.4); }
`;

// ─── 3D Card Component ────────────────────────────────────────────────────────
function Arsenal3DCard({
  mod, isEnabled, onToggle, onLaunch, onClose, onSelectDetail, index
}: {
  mod: ArsenalModule;
  isEnabled: boolean;
  ...
  onLaunch: (id: ArsenalModuleId) => void;
  onClose: () => void;
  onSelectDetail: (mod: ArsenalModule) => void;
  index: number;
}) {
  ...
  const rotateX = useSpring(useTransform(mouseY, [-0.5, 0.5], [6, -6]), { stiffness: 320, damping: 35 });
  const rotateY = useSpring(useTransform(mouseX, [-0.5, 0.5], [-6, 6]), { stiffness: 320, damping: 35 });
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: isEnabled ? 1 : 0.38, y: 0, scale: 1 }}
      transition={{ duration: 0.38, delay: Math.min(index * 0.016, 0.45), type: "spring", stiffness: 220, damping: 24 }}
      ref={cardRef}
      onMouseMove={handleMouseMove}
      onMouseLeave={handleMouseLeave}
      style={{ perspective: "1000px", transformStyle: "preserve-3d" }}
      className="relative group arsenal-card"
    >
      <motion.div
      ...
      >
        ...
        <div className="absolute inset-0" style={{
          background: isEnabled
            ? `radial-gradient(ellipse at 25% 15%, ${mod.color}1a 0%, transparent 55%),
               linear-gradient(145deg, rgba(15,15,20,0.98) 0%, rgba(6,6,10,0.99) 100%)`
            : "linear-gradient(145deg, rgba(8,8,12,0.98) 0%, rgba(4,4,7,0.99) 100%)",
          zIndex: 0
        }} />
        ...
        <div className="absolute top-0 inset-x-0 h-px" style={{
          background: isEnabled
            ? `linear-gradient(90deg, transparent 5%, ${mod.color}bb 40%, ${mod.color}ee 50%, ${mod.color}bb 60%, transparent 95%)`
            : "linear-gradient(90deg, transparent, rgba(255,255,255,0.04), transparent)",
          boxShadow: isEnabled ? `0 0 12px ${mod.color}55` : "none",
          zIndex: 2
        }} />

        {/* ── Corner brackets ── */}
        <div className="absolute top-0 left-0 w-5 h-5 pointer-events-none" style={{
          borderTop: `1.5px solid ${isEnabled ? mod.color + "70" : "rgba(255,255,255,0.05)"}`,
          borderLeft: `1.5px solid ${isEnabled ? mod.color + "70" : "rgba(255,255,255,0.05)"}`,
          borderRadius: "12px 0 0 0",
          zIndex: 3
        }} />
        <div className="absolute bottom-0 right-0 w-5 h-5 pointer-events-none" style={{
          borderBottom: `1.5px solid ${isEnabled ? mod.color + "45" : "rgba(255,255,255,0.03)"}`,
          borderRight: `1.5px solid ${isEnabled ? mod.color + "45" : "rgba(255,255,255,0.03)"}`,
        ...
        <div className="absolute inset-0 rounded-2xl pointer-events-none" style={{
          border: `1px solid ${isEnabled ? mod.border : "rgba(255,255,255,0.055)"}`,
          boxShadow: isEnabled
            ? `0 0 0 1px ${mod.color}08, 0 4px 24px rgba(0,0,0,0.8), 0 0 32px ${mod.glow}, inset 0 1px 0 rgba(255,255,255,0.05)`
        ...
        {isEnabled && (
          <motion.div className="absolute inset-0 rounded-2xl pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-400" style={{
            background: `radial-gradient(circle at ${glowX} ${glowY}, ${mod.color}0e 0%, transparent 65%)`,
            zIndex: 1
          }} />
        )}
          ...
          <div className="flex items-start gap-3">
            ...
            >
              <div className="absolute inset-0 rounded-xl" style={{
                background: isEnabled
                  ? `radial-gradient(circle at 35% 30%, ${mod.color}38 0%, rgba(0,0,0,0.92) 75%)`
                  : "rgba(255,255,255,0.025)",
                border: `1px solid ${isEnabled ? mod.border : "rgba(255,255,255,0.06)"}`,
                boxShadow: isEnabled
                  ? `0 0 20px ${mod.glow}, inset 0 1px 0 rgba(255,255,255,0.09), 0 4px 14px rgba(0,0,0,0.55)`
                  : "none"
              }} />
              <Icon style={{
                ...
                filter: isEnabled ? `drop-shadow(0 0 7px ${mod.color})` : "none"
              }} />
              {isEnabled && (
                <div className="absolute inset-0 rounded-xl pointer-events-none" style={{ animation: "arsenal-spin-slow 5s linear infinite" }}>
                  <div className="absolute top-0.5 left-1/2 w-[3px] h-[3px] rounded-full -translate-x-1/2" style={{
                    background: mod.color,
                    boxShadow: `0 0 5px ${mod.color}, 0 0 10px ${mod.color}80`
                  }} />
                </div>
              )}
            </motion.div>
            ...
            <div className="flex-1 min-w-0 pt-0.5">
              <div className="flex items-center gap-2 flex-wrap mb-0.5">
                <span className="text-[12.5px] font-black tracking-wide leading-none" style={{
                  color: isEnabled ? "#f1f5f9" : "#292929",
                  textShadow: isEnabled ? `0 0 16px ${mod.color}55` : "none",
                }}>
                  {mod.name}
                </span>
                <span className="text-[6px] font-black px-1.5 py-[2px] rounded-full font-mono tracking-[0.12em] uppercase" style={{
                  background: isEnabled ? `${mod.color}1c` : "rgba(255,255,255,0.03)",
                  border: `1px solid ${isEnabled ? `${mod.color}50` : "rgba(255,255,255,0.05)"}`,
                  color: isEnabled ? mod.color : "#222",
                }}>
                  {mod.tag}
                </span>
              </div>
              <div className="text-[9px] font-mono leading-snug" style={{
                color: isEnabled ? "rgba(255,255,255,0.42)" : "#1e1e1e"
              }}>
                {mod.subtitle}
              </div>
            </div>
            ...
            <button
              ...
              className="relative flex-shrink-0 w-9 h-[18px] rounded-full transition-all duration-200"
              style={{
                background: isEnabled
                  ? `linear-gradient(90deg, ${mod.color}dd, ${mod.color}ff)`
                  : "rgba(255,255,255,0.06)",
                border: `1px solid ${isEnabled ? mod.border : "rgba(255,255,255,0.08)"}`,
                boxShadow: isEnabled ? `0 0 12px ${mod.glow}` : "none"
              }}
            >
              <div className="absolute top-[2px] w-[13px] h-[13px] rounded-full shadow-md transition-all duration-200"
                style={{
                  left: isEnabled ? "calc(100% - 15px)" : 2,
                  background: isEnabled ? "#fff" : "rgba(255,255,255,0.25)",
                  boxShadow: isEnabled ? `0 0 6px ${mod.color}90` : "none"
                }} />
            </button>
          </div>

          {/* Description */}
          <p className="text-[9.5px] leading-[1.55] font-sans" style={{
            color: isEnabled ? "rgba(255,255,255,0.52)" : "#161616"
          }}>
            {mod.desc}
          </p>

          {/* Footer row */}
          <div className="flex items-center justify-between pt-2.5 border-t" style={{
            borderColor: isEnabled ? `${mod.color}1a` : "rgba(255,255,255,0.04)"
          }}>
            {/* Source badge */}
            <div className="flex items-center gap-1.5 min-w-0">
              <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{
                background: isEnabled ? mod.color : "#1e1e1e",
                boxShadow: isEnabled ? `0 0 5px ${mod.color}` : "none"
              }} />
              <span className="text-[8px] font-mono truncate" style={{
                color: isEnabled ? "rgba(255,255,255,0.2)" : "#101010"
              }}>
                {mod.source.length > 24 ? mod.source.slice(0, 24) + "…" : mod.source}
              </span>
            </div>
            ...
            <motion.button
              ...
              disabled={!isEnabled}
              className="arsenal-card-launch flex items-center gap-1.5 px-3 py-[5px] rounded-lg text-[9.5px] font-black disabled:opacity-15 tracking-wider transition-all"
              style={{
                background: isEnabled
                  ? `linear-gradient(135deg, ${mod.color}20, ${mod.color}30)`
                  : "rgba(255,255,255,0.02)",
                border: `1px solid ${isEnabled ? mod.border : "rgba(255,255,255,0.04)"}`,
                color: isEnabled ? mod.color : "#111",
                boxShadow: isEnabled ? `0 0 16px ${mod.glow}, 0 2px 6px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.06)` : "none",
                textShadow: isEnabled ? `0 0 10px ${mod.color}80` : "none"
              }}
            >
...
// ─── Tab button ───────────────────────────────────────────────────────────────
function TabBtn({ active, onClick, label, count, countColor = "#6b7280" }: {
  active: boolean; onClick: () => void; label: string; count?: number; countColor?: string;
}) {
  return (
    <button
      onClick={onClick}
      className="relative px-4 py-3 text-[9px] font-black tracking-[0.12em] transition-all duration-200 whitespace-nowrap group"
      style={{ color: active ? "#fb7185" : "#5a6478" }}
    >
      ...
      {active && (
        <motion.div
          ...
          style={{
            background: "linear-gradient(90deg, transparent, #e21227cc, #e21227, #e21227cc, transparent)",
            boxShadow: "0 0 10px rgba(226,18,39,0.7), 0 0 20px rgba(226,18,39,0.3)"
          }}
      ...
      {active && (
        <div className="absolute inset-0 pointer-events-none rounded-t-lg" style={{
          background: "linear-gradient(180deg, rgba(226,18,39,0.04) 0%, transparent 100%)"
        }} />
      )}
      {/* Hover bg */}
      {!active && (
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-150 rounded-t-lg"
          style={{ background: "rgba(255,255,255,0.025)" }} />
      )}
      <span className="relative z-10 flex items-center gap-1.5">
        {label}
        {count !== undefined && count > 0 && (
          <span className="text-[7.5px] font-mono px-[5px] py-[1px] rounded-full" style={{
            background: active ? "rgba(226,18,39,0.18)" : "rgba(255,255,255,0.06)",
            border: `1px solid ${active ? "rgba(226,18,39,0.35)" : "rgba(255,255,255,0.08)"}`,
            color: active ? "#fb7185" : countColor,
            lineHeight: 1.6
          }}>
            {count}
          </span>
...
export function ArsenalHubModal({ open, onOpenChange, onLaunch }: ArsenalHubModalProps) {
  ...
  return (
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          <motion.div
            initial={{ opacity: 0, scale: 0.97, y: 24, rotateX: 3 }}
            animate={{ opacity: 1, scale: 1, y: 0, rotateX: 0 }}
            exit={{ opacity: 0, scale: 0.97, y: 24 }}
            transition={{ duration: 0.28, type: "spring", stiffness: 200, damping: 28 }}
            className="w-full h-[96vh] flex flex-col rounded-[18px] overflow-hidden relative"
            style={{
          ...
          >
            ...
            <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-2xl" style={{ zIndex: 0 }}>
              {/* Fine dot grid */}
              <svg className="absolute inset-0 w-full h-full" style={{ opacity: 0.028 }} preserveAspectRatio="xMidYMid slice">
                <defs>
                  <pattern id="hx" x="0" y="0" width="44" height="50.8" patternUnits="userSpaceOnUse">
                    <polygon points="22,2 42,13 42,37 22,48 2,37 2,13" fill="none" stroke="#e21227" strokeWidth="0.5" />
                  </pattern>
                </defs>
              ...
              {/* Corner accent brackets — crisp engineering corners */}
              <div className="absolute top-3.5 left-3.5 w-10 h-10 pointer-events-none" style={{ borderTop: "1px solid rgba(226,18,39,0.4)", borderLeft: "1px solid rgba(226,18,39,0.4)", borderRadius: "6px 0 0 0" }} />
              <div className="absolute top-3.5 right-3.5 w-10 h-10 pointer-events-none" style={{ borderTop: "1px solid rgba(226,18,39,0.4)", borderRight: "1px solid rgba(226,18,39,0.4)", borderRadius: "0 6px 0 0" }} />
              <div className="absolute bottom-3.5 left-3.5 w-10 h-10 pointer-events-none" style={{ borderBottom: "1px solid rgba(226,18,39,0.18)", borderLeft: "1px solid rgba(226,18,39,0.18)", borderRadius: "0 0 0 6px" }} />
              <div className="absolute bottom-3.5 right-3.5 w-10 h-10 pointer-events-none" style={{ borderBottom: "1px solid rgba(226,18,39,0.18)", borderRight: "1px solid rgba(226,18,39,0.18)", borderRadius: "0 0 6px 0" }} />
              {/* Top-center aurora */}
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-40 pointer-events-none" style={{ background: "radial-gradient(ellipse at 50% 0%, rgba(226,18,39,0.09) 0%, transparent 70%)" }} />
              {/* Bottom-left subtle leak */}
              <div className="absolute bottom-0 left-0 w-72 h-32 pointer-events-none" style={{ background: "radial-gradient(ellipse at 0% 100%, rgba(0,229,204,0.03) 0%, transparent 70%)" }} />
            </div>

            {/* ══════════════════════════════════════════
                HEADER
            ══════════════════════════════════════════ */}
            <div className="relative z-10 border-b" style={{ borderColor: "rgba(255,255,255,0.07)", background: "linear-gradient(180deg, rgba(226,18,39,0.04) 0%, rgba(0,0,0,0.0) 100%)" }}>
              {/* Top strip */}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="relative flex-shrink-0">
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{
                      background: "radial-gradient(circle at 32% 28%, rgba(226,18,39,0.28) 0%, rgba(6,6,10,0.94) 70%)",
                      border: "1px solid rgba(226,18,39,0.45)",
                      boxShadow: "0 0 28px rgba(226,18,39,0.28), inset 0 1px 0 rgba(255,255,255,0.07)"
                    }}>
                      <motion.div animate={{ rotate: [0, 360] }} transition={{ duration: 22, repeat: Infinity, ease: "linear" }}
                        style={{ position: "absolute", inset: 4, borderRadius: "50%", border: "1px dashed rgba(226,18,39,0.22)" }} />
                      <Shield className="w-5 h-5 relative z-10" style={{ color: "#f87171", filter: "drop-shadow(0 0 8px #e21227)" }} />
                    </div>
                    {/* Live pulse dot */}
                    <div className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full flex items-center justify-center" style={{ background: "#10b981", boxShadow: "0 0 10px #10b981, 0 0 20px rgba(16,185,129,0.4)" }}>
                      <motion.div animate={{ scale: [1, 1.5, 1], opacity: [1, 0.4, 1] }} transition={{ duration: 1.8, repeat: Infinity }}
                  ...
                  <div>
                    <div className="flex items-center gap-3 flex-wrap">
                      <span className="text-[17px] font-black tracking-[0.18em] leading-none" style={{
                        color: "#f87171",
                        textShadow: "0 0 20px rgba(226,18,39,0.55), 0 0 42px rgba(226,18,39,0.22)",
                        animation: "arsenal-title-glow 3.5s ease-in-out infinite"
                      }}>ARSENAL</span>
                      <span className="text-[17px] font-black tracking-[0.18em] leading-none" style={{ color: "rgba(255,255,255,0.92)" }}>HUB</span>
                      {/* Version badge */}
                      <span className="text-[7px] font-black px-2 py-[3px] rounded-md font-mono tracking-[0.15em]" style={{
                        color: "#22d3ee",
                      ...
                      {/* LIVE badge */}
                      <span className="flex items-center gap-1.5 text-[7px] font-black px-2 py-[3px] rounded-md font-mono tracking-widest" style={{
                        color: "#4ade80",
                        border: "1px solid rgba(74,222,128,0.3)",
                        background: "rgba(74,222,128,0.07)"
                      }}>
                        <motion.div animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 1.2, repeat: Infinity }}
                          className="w-1.5 h-1.5 rounded-full" style={{ background: "#4ade80", boxShadow: "0 0 5px #4ade80" }} />
                        LIVE
                      </span>
                    </div>
                    <div className="flex items-center gap-2.5 mt-1.5">
                      <span className="text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.32)" }}>
                        <span style={{ color: "rgba(255,255,255,0.7)", fontWeight: 900 }}>{enabled.size}</span>
                        <span style={{ color: "rgba(255,255,255,0.2)" }}>/{ARSENAL_MODULES.length} modules active</span>
                      </span>
                      <span className="text-[7.5px] font-mono px-2 py-[2px] rounded-full" style={{
                ...
                <div className="flex items-center gap-2.5">
                  {tab === "modules" && (
                    <motion.button
                      ...
                      onClick={toggleAll}
                      className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-[10px] font-black transition-all border tracking-widest"
                      style={{
                        background: allOn ? "rgba(226,18,39,0.1)" : "rgba(34,211,238,0.07)",
                        borderColor: allOn ? "rgba(226,18,39,0.32)" : "rgba(34,211,238,0.28)",
                        color: allOn ? "#fb7185" : "#22d3ee",
                        boxShadow: allOn ? "0 0 14px rgba(226,18,39,0.15)" : "0 0 14px rgba(34,211,238,0.12)"
                      }}
                    >
                  ...
                  {tab === "history" && history.length > 0 && (
                    <motion.button
                      ...
                      className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-[10px] font-black border tracking-widest"
                      style={{ background: "rgba(248,113,113,0.07)", borderColor: "rgba(248,113,113,0.28)", color: "#f87171" }}
                    >
              ...
              {/* ── Stats strip ── */}
              <div className="grid grid-cols-4 gap-0 border-t mx-0" style={{ borderColor: "rgba(255,255,255,0.055)" }}>
                {[
                  { label: "MODULES", value: ARSENAL_MODULES.length, color: "#fb7185", glow: "rgba(226,18,39,0.2)" },
                  { label: "ACTIVE",  value: enabled.size,           color: "#4ade80", glow: "rgba(74,222,128,0.2)" },
                  { label: "CHAIN RULES", value: rulesCount,         color: "#22d3ee", glow: "rgba(34,211,238,0.2)" },
                  { label: "PIPELINE",    value: history.length,     color: "#fbbf24", glow: "rgba(251,191,36,0.2)" },
                ].map((s, i) => (
                  <div key={s.label} className="flex items-center gap-3 px-5 py-2.5 relative"
                    style={{ borderRight: i < 3 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                    <span className="text-2xl font-black font-mono leading-none" style={{ color: s.color, textShadow: `0 0 14px ${s.glow}` }}>{s.value}</span>
                    <span className="text-[8px] font-black tracking-widest font-mono" style={{ color: "rgba(255,255,255,0.28)" }}>{s.label}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* ══════════════════════════════════════════
                TAB BAR
            ══════════════════════════════════════════ */}
            <div className="relative z-10 flex items-center px-2 border-b overflow-x-auto" style={{
              borderColor: "rgba(255,255,255,0.06)",
              background: "rgba(0,0,0,0.35)",
              scrollbarWidth: "none"
            }}>
              <TabBtn active={tab === "modules"}  onClick={() => setTab("modules")}  label="MODULES"       count={ARSENAL_MODULES.length} />
              <TabBtn active={tab === "chain"}    onClick={() => setTab("chain")}    label="CHAIN BUILDER" count={rulesCount} countColor="#22d3ee" />
              <TabBtn active={tab === "history"}  onClick={() => setTab("history")}  label="PIPELINE"      count={history.length} countColor="#22d3ee" />
              <TabBtn active={tab === "mission"}  onClick={() => setTab("mission")}  label="MISSION CTRL" />
              <TabBtn active={tab === "intel"}    onClick={() => setTab("intel")}    label="INTEL FEED"    countColor="#fb7185" />
            ...
            ══════════════════════════════════════════ */}
            {tab === "modules" && (
              <div className="relative z-10 px-5 py-3 border-b" style={{ borderColor: "rgba(255,255,255,0.05)", background: "rgba(0,0,0,0.22)" }}>
                <div className="relative">
                  {/* Search icon */}
                  <div className="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none">
                    <Search className="w-4 h-4" style={{ color: search ? "rgba(226,18,39,0.7)" : "rgba(255,255,255,0.2)" }} />
                  </div>
                  <input
                    type="text"
                    value={search}
                    ...
                    className="arsenal-input w-full border rounded-xl pl-10 pr-28 py-2.5 text-[11px] transition-all duration-200 font-sans"
                    style={{
                      background: "rgba(255,255,255,0.03)",
                      borderColor: search ? "rgba(226,18,39,0.38)" : "rgba(255,255,255,0.08)",
                      color: "rgba(255,255,255,0.82)",
                      boxShadow: search ? "0 0 0 3px rgba(226,18,39,0.07)" : "none"
                    }}
                  />
                  {/* Right side: found count + shortcut hint */}
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                    {search ? (
                      <AnimatePresence>
                        <motion.span key="found" initial={{ opacity: 0, x: 4 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -4 }}
                          className="text-[9px] font-black font-mono" style={{ color: "#fb7185" }}>
                          {filtered.length} found
                        </motion.span>
                      </AnimatePresence>
                    ) : (
                      <span className="hidden sm:flex items-center gap-1 text-[8px] font-mono px-2 py-[3px] rounded-md" style={{
            ...
            {/* ══════════════════════════════════════════
                CONTENT
            ══════════════════════════════════════════ */}
            <div className="flex-1 overflow-y-auto relative z-10 arsenal-scrollbar">
              <AnimatePresence mode="wait">

                {/* ── MODULES GRID ── */}
                {tab === "modules" && (
                  <motion.div key="modules" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18 }} className="p-5">
                    {filtered.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-24 gap-4">
                        <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{
                          background: "rgba(226,18,39,0.06)",
                          border: "1px solid rgba(226,18,39,0.18)",
                          boxShadow: "0 0 30px rgba(226,18,39,0.06)"
                        }}>
                          <Shield className="w-7 h-7" style={{ color: "rgba(226,18,39,0.3)" }} />
                        </div>
                        <div className="text-center">
                          <div className="text-[12px] font-black tracking-widest font-mono mb-1" style={{ color: "rgba(255,255,255,0.25)" }}>NO MODULES FOUND</div>
                          <div className="text-[10px] font-mono" style={{ color: "rgba(255,255,255,0.1)" }}>Try a different search term</div>
                        </div>
                      </div>
                    ) : (
                ...
                {/* ── CHAIN BUILDER ── */}
                {tab === "chain" && (
                  <motion.div key="chain" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18 }}>
                    <ChainBuilderTab />
                  </motion.div>
                )}
                ...
                {/* ── MISSION CONTROL ── */}
                {tab === "mission" && (
                  <motion.div key="mission" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18 }} className="p-5 space-y-5">

                    {/* Stat cards row */}
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {[
                        { label: "TOTAL MODULES", value: ARSENAL_MODULES.length, color: "#fb7185", glow: "rgba(226,18,39,0.25)", icon: <Layers style={{ width: 15, height: 15 }} /> },
                        { label: "ONLINE",         value: enabled.size,           color: "#4ade80", glow: "rgba(74,222,128,0.25)",  icon: <Activity style={{ width: 15, height: 15 }} /> },
                        { label: "CHAIN RULES",    value: rulesCount,             color: "#22d3ee", glow: "rgba(34,211,238,0.25)", icon: <GitMerge style={{ width: 15, height: 15 }} /> },
                        { label: "PIPELINE EVT",   value: history.length,         color: "#fbbf24", glow: "rgba(251,191,36,0.25)",  icon: <BarChart2 style={{ width: 15, height: 15 }} /> },
                      ].map((s, i) => (
                        <motion.div
                          key={s.label}
                          ...
                          style={{
                            background: `radial-gradient(ellipse at 25% 20%, ${s.color}14 0%, rgba(6,6,10,0.92) 70%)`,
                            border: `1px solid ${s.color}28`,
                            boxShadow: `0 0 24px ${s.glow}, inset 0 1px 0 rgba(255,255,255,0.04)`
                          }}
                        >
                          <div className="absolute top-0 inset-x-0 h-px" style={{ background: `linear-gradient(90deg, transparent, ${s.color}88, transparent)` }} />
                          <div className="flex items-center gap-2 mb-3" style={{ color: s.color }}>
                            {s.icon}
                            <span className="text-[7.5px] font-black tracking-[0.16em] font-mono">{s.label}</span>
                          </div>
                          <div className="text-[32px] font-mono font-black leading-none" style={{ color: s.color, textShadow: `0 0 24px ${s.glow}` }}>{s.value}</div>
                        </motion.div>
                      ))}
                    </div>

                    {/* Module status matrix */}
                    <div className="rounded-2xl border overflow-hidden" style={{ background: "rgba(8,8,12,0.7)", borderColor: "rgba(255,255,255,0.07)" }}>
                      <div className="px-4 py-3 border-b flex items-center gap-2.5" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                        <div className="w-2 h-2 rounded-full" style={{ background: "#fb7185", boxShadow: "0 0 8px rgba(226,18,39,0.5)" }} />
                        <span className="text-[9px] font-black tracking-[0.15em] font-mono" style={{ color: "#fb7185" }}>MODULE STATUS MATRIX</span>
                        <span className="ml-auto text-[8px] font-mono" style={{ color: "rgba(255,255,255,0.2)" }}>{enabled.size}/{ARSENAL_MODULES.length} active</span>
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 p-3 max-h-64 overflow-y-auto arsenal-scrollbar">
                        {ARSENAL_MODULES.map(mod => {
                          ...
                          return (
                            <div key={mod.id} className="flex items-center gap-2 px-2.5 py-2 rounded-xl border transition-all"
                              style={{ borderColor: isOn ? `${mod.color}28` : "rgba(255,255,255,0.05)", background: isOn ? `${mod.color}0a` : "rgba(255,255,255,0.02)" }}>
                              <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style={{
                                background: isOn ? `${mod.color}20` : "rgba(255,255,255,0.04)",
                                border: `1px solid ${isOn ? mod.border : "rgba(255,255,255,0.05)"}`,
                              }}>
                                <Icon style={{ width: 13, height: 13, color: isOn ? mod.color : "#2e2e2e", filter: isOn ? `drop-shadow(0 0 4px ${mod.color})` : "none" }} />
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="text-[9px] font-bold truncate" style={{ color: isOn ? "rgba(255,255,255,0.75)" : "#2a2a2a" }}>{mod.name}</div>
                                <div className="text-[7px] font-mono" style={{ color: isOn ? "rgba(255,255,255,0.22)" : "#1a1a1a" }}>{lastEvent ? `fired: ${lastEvent.label.slice(0,16)}` : "idle"}</div>
                              </div>
                              <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: isOn ? "#4ade80" : "#1a1a1a", boxShadow: isOn ? "0 0 5px #4ade80" : "none" }} />
                            </div>
                          );
                        })}
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <div className="rounded-2xl border overflow-hidden" style={{ background: "rgba(8,8,12,0.7)", borderColor: "rgba(255,255,255,0.07)" }}>
                        <div className="px-4 py-3 border-b flex items-center gap-2" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                          <div className="w-1.5 h-1.5 rounded-full" style={{ background: "#22d3ee", boxShadow: "0 0 6px rgba(34,211,238,0.5)" }} />
                          <span className="text-[9px] font-black tracking-[0.15em] font-mono" style={{ color: "#22d3ee" }}>RECENT EVENTS</span>
                        </div>
                        {history.length === 0 ? (
                          <div className="text-center py-8 text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.15)" }}>No events yet</div>
                        ) : (
                          <div className="space-y-0.5 p-2 max-h-36 overflow-y-auto arsenal-scrollbar">
                            {history.slice(0, 8).map(entry => (
                              <div key={entry.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/[0.02] transition-colors">
                                <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: entry.sourceColor, boxShadow: `0 0 4px ${entry.sourceColor}` }} />
                                <span className="text-[8.5px] font-bold font-mono truncate flex-1" style={{ color: "rgba(255,255,255,0.55)" }}>{entry.source}</span>
                                <span className="text-[7px] font-mono flex-shrink-0" style={{ color: "rgba(255,255,255,0.22)" }}>{new Date(entry.timestamp).toLocaleTimeString("en-US", { hour12: false })}</span>
                              </div>
                            ))}
                          </div>
                        )}
                      ...
                      <div className="rounded-2xl border overflow-hidden" style={{ background: "rgba(8,8,12,0.7)", borderColor: "rgba(255,255,255,0.07)" }}>
                        <div className="px-4 py-3 border-b flex items-center gap-2" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                          <div className="w-1.5 h-1.5 rounded-full" style={{ background: "#a78bfa", boxShadow: "0 0 6px rgba(167,139,250,0.5)" }} />
                          <span className="text-[9px] font-black tracking-[0.15em] font-mono" style={{ color: "#a78bfa" }}>CATEGORIES</span>
                        </div>
                        <div className="flex flex-wrap gap-1.5 p-3 max-h-36 overflow-y-auto arsenal-scrollbar">
                          {Array.from(new Set(ARSENAL_MODULES.map(m => m.tag))).map(tag => {
                            ...
                            const active = cnt > 0;
                            return (
                              <div key={tag} className="flex items-center gap-1.5 px-2 py-[3px] rounded-full text-[7px] font-black tracking-wide"
                                style={{
                                  background: active ? "rgba(34,211,238,0.09)" : "rgba(255,255,255,0.03)",
                                  border: `1px solid ${active ? "rgba(34,211,238,0.22)" : "rgba(255,255,255,0.06)"}`,
                                  color: active ? "#22d3ee" : "rgba(255,255,255,0.2)"
                                }}>
                                {tag}
                                <span style={{ opacity: 0.65, fontWeight: 400 }}>{cnt}/{tot}</span>
                              </div>
                            );
                          })}
                        </div>
                ...
                {/* ── INTEL FEED ── */}
                {tab === "intel" && (
                  <motion.div key="intel" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18 }} className="p-5">
                    <IntelFeedTab />
                  </motion.div>
                )}
                ...
                {/* ── COUNCIL 105 ── */}
                {tab === "council" && (
                  <motion.div key="council" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18 }} className="flex flex-col h-full" style={{ minHeight: "500px" }}>
                ...
                {/* ── PIPELINE HISTORY ── */}
                {tab === "history" && (
                  <motion.div key="history" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18 }} className="p-5">
                    {history.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-24 gap-4">
                        <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{
                          ...
                          <GitMerge className="w-7 h-7" style={{ color: "rgba(34,211,238,0.3)" }} />
                        </div>
                        <div className="text-center">
                          <div className="text-[12px] font-black tracking-widest font-mono mb-1" style={{ color: "rgba(255,255,255,0.2)" }}>PIPELINE EMPTY</div>
                          <div className="text-[10px] font-mono" style={{ color: "rgba(255,255,255,0.1)" }}>Use Pipe buttons in any module to route output here</div>
                        </div>
                      </div>
                    ) : (
                      <div className="flex flex-col gap-2.5">
                        {history.map((entry, idx) => (
                          <motion.div key={entry.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.02 }}
                            className="rounded-2xl p-4 relative overflow-hidden"
                            style={{
                              background: `linear-gradient(145deg, rgba(10,10,16,0.98) 0%, rgba(6,6,10,0.99) 100%)`,
                              border: `1px solid ${entry.sourceColor}1e`,
                              boxShadow: `0 2px 16px rgba(0,0,0,0.4)`
                            }}>
                            <div className="absolute top-0 inset-x-0 h-px" style={{ background: `linear-gradient(90deg, transparent, ${entry.sourceColor}70, transparent)` }} />
                            <div className="flex items-center gap-2 mb-3">
                              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg" style={{ background: `${entry.sourceColor}14`, border: `1px solid ${entry.sourceColor}28` }}>
                                <div className="w-1.5 h-1.5 rounded-full" style={{ background: entry.sourceColor, boxShadow: `0 0 5px ${entry.sourceColor}` }} />
                                <span className="text-[9px] font-black font-mono" style={{ color: entry.sourceColor }}>{entry.source}</span>
                              </div>
                              <ArrowRight className="w-3 h-3 flex-shrink-0" style={{ color: entry.destination ? "rgba(34,211,238,0.6)" : "rgba(255,255,255,0.12)" }} />
                              {entry.destination ? (
                                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg" style={{ background: `${entry.destinationColor}14`, border: `1px solid ${entry.destinationColor}28` }}>
                                  <div className="w-1.5 h-1.5 rounded-full" style={{ background: entry.destinationColor ?? "#22d3ee" }} />
                                  <span className="text-[9px] font-black font-mono" style={{ color: entry.destinationColor ?? "#22d3ee" }}>{entry.destination}</span>
                                </div>
                              ) : (
                                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}>
                                  <span className="text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.2)" }}>pending</span>
                                </div>
                              )}
                              <span className="ml-auto text-[7.5px] font-mono" style={{ color: "rgba(255,255,255,0.2)" }}>{entry.timestamp}</span>
                            </div>
                            <div className="mb-2.5">
                              <span className="text-[8px] font-bold font-mono px-2.5 py-1 rounded-lg" style={{
                                background: "rgba(255,255,255,0.05)",
                                border: "1px solid rgba(255,255,255,0.08)",
                                color: "rgba(255,255,255,0.45)"
                              }}>{entry.label}</span>
                            </div>
                            <div className="text-[9.5px] font-mono leading-relaxed mb-3 line-clamp-3" style={{ color: "rgba(255,255,255,0.38)", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
                            ...
                            <div className="flex items-center gap-2 pt-2 border-t" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
                              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                                ...
                                className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] font-black border transition-all"
                                style={{
                                  background: replayedId === entry.id ? "rgba(34,211,238,0.14)" : "rgba(34,211,238,0.06)",
                                  borderColor: replayedId === entry.id ? "rgba(34,211,238,0.4)" : "rgba(34,211,238,0.16)",
                                  color: replayedId === entry.id ? "#22d3ee" : "rgba(34,211,238,0.45)"
                                }}>
                              ...
                              <div className="ml-auto text-[7.5px] font-mono" style={{ color: "rgba(255,255,255,0.18)" }}>{entry.content.length} chars</div>
                            </div>
                          </motion.div>
                        ))}
                      </div>
            ...
            {/* ══════════════════════════════════════════
                FOOTER
            ══════════════════════════════════════════ */}
            <div className="relative z-10 px-5 py-2.5 border-t flex items-center justify-between" style={{
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {[
                  { val: ARSENAL_MODULES.length, label: "modules",   color: "#fb7185" },
                  { val: pipeline.getRules().length, label: "rules",  color: "#22d3ee" },
                  { val: history.length,          label: "events",   color: "#fbbf24" },
                ].map(s => (
                  <div key={s.label} className="flex items-center gap-1.5 px-2.5 py-[3px] rounded-full" style={{
                    background: "rgba(255,255,255,0.04)",
                    border: "1px solid rgba(255,255,255,0.07)"
                  }}>
                    <span className="text-[10px] font-black font-mono" style={{ color: s.color }}>{s.val}</span>
                    <span className="text-[8px] font-mono" style={{ color: "rgba(255,255,255,0.28)" }}>{s.label}</span>
                  </div>
                ))}
              </div>
              {/* Right: brand */}
              <div className="flex items-center gap-2">
                <motion.div animate={{ opacity: [0.4, 0.85, 0.4] }} transition={{ duration: 2.8, repeat: Infinity }}>
                  <Brain className="w-3.5 h-3.5" style={{ color: "rgba(226,18,39,0.5)" }} />
                </motion.div>
                <span className="text-[8px] font-black font-mono tracking-widest" style={{ color: "rgba(255,255,255,0.22)" }}>KaliGPT</span>
                <span className="text-[7px] font-mono px-1.5 py-[2px] rounded" style={{ background: "rgba(226,18,39,0.1)", border: "1px solid rgba(226,18,39,0.18)", color: "rgba(226,18,39,0.55)" }}>v3090</span>
              </div>
            </div>
          </motion.div>
        </motion.div>
    ...
    {/* ── ToolDetailView overlay ── */}
    {selectedMod && (
      <ToolDetailView
        module={selectedMod}
        isEnabled={enabled.has(selectedMod.id)}
        onClose={() => setSelectedMod(null)}
        onLaunch={(id) => { onLaunch(id); onOpenChange(false); }}
      />
    )}
    </>
  );
