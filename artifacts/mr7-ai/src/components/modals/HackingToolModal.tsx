  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)" }}>
      <motion.div
        ...
        style={{ width: "clamp(340px, 42vw, 580px)", backdropFilter: "blur(40px)", background: "linear-gradient(160deg, rgba(12,2,4,0.97) 0%, rgba(7,1,2,0.98) 100%)", border: `1px solid ${Rg(0.28)}`, maxHeight: "90vh", boxShadow: `0 0 60px ${Rg(0.08)}, 0 32px 64px rgba(0,0,0,0.6), inset 0 1px 0 ${Rg(0.08)}` }}
      >
        <div className="flex items-center justify-between px-4 pt-3 pb-[10px] border-b flex-shrink-0" style={{ borderColor: Rg(0.12) }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px]" style={{ color: "rgba(255,255,255,0.42)" }}>185+ Security Tools · 20 Categories · AI-Powered Usage Guide</div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="relative">
              <Search className="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2" style={{ color: "rgba(255,255,255,0.4)" }} />
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-48 border-r overflow-y-auto flex-shrink-0 py-2" style={{ borderColor: Rg(0.08) }}>
            <button onClick={() => setSelectedCat(null)} className="w-full text-left px-3 py-2 text-[10px] font-bold transition-all"
              style={{ color: !selectedCat ? R : "rgba(255,255,255,0.45)", background: !selectedCat ? Rg(0.06) : "transparent" }}>
            ...
            {CATEGORIES.map(cat => (
              <button key={cat.id} onClick={() => setSelectedCat(selectedCat === cat.id ? null : cat.id)}
                ...
                style={{ color: selectedCat === cat.id ? cat.color : "rgba(255,255,255,0.45)", background: selectedCat === cat.id ? `${cat.color}10` : "transparent" }}>
                <cat.icon className="w-3 h-3 flex-shrink-0" style={{ color: cat.color }} />
                <span className="truncate">{cat.name}</span>
                <span className="ml-auto text-[9px]" style={{ color: "rgba(255,255,255,0.3)" }}>{cat.count}</span>
              </button>
            ))}
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {!selectedTool ? (
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {cat.tools.map(tool => (
                        <button key={tool.name} onClick={() => { setSelectedTool(tool); setAiResponse(""); setAiQuery(""); }}
                          ...
                          <div className="text-[9px] leading-relaxed mb-2" style={{ color: "rgba(255,255,255,0.48)" }}>{tool.desc.slice(0, 70)}...</div>
              ...
              <div className="space-y-4">
                <button onClick={() => { setSelectedTool(null); setAiResponse(""); }} className="flex items-center gap-2 text-[10px]" style={{ color: "rgba(255,255,255,0.45)" }}>
                  ← Back to tools
                </button>
                <div className="rounded-xl p-4 relative overflow-hidden" style={{ background: "rgba(226,18,39,0.05)", border: `1px solid ${Rg(0.25)}`, backdropFilter: "blur(12px)" }}>
                  <div className="absolute inset-x-0 top-0 h-px" style={{ background: `linear-gradient(90deg, transparent, ${Rg(0.8)}, transparent)` }} />
                  <div className="text-[15px] font-black font-mono text-white mb-1">{selectedTool.name}</div>
                  <div className="text-[11px] mb-3" style={{ color: "rgba(255,255,255,0.58)" }}>{selectedTool.desc}</div>
                  <div className="flex flex-wrap gap-1">
                ...
                {aiResponse && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-xl p-4 text-[11px] leading-relaxed whitespace-pre-wrap font-mono" style={{ background: "rgba(0,0,0,0.55)", border: `1px solid ${Rg(0.14)}`, color: "rgba(255,255,255,0.68)" }}>
                    {aiResponse}
                  </motion.div>
                )}
              </div>
