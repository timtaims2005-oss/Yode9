...
Be thorough and produce production-ready output.`;
    ...
    try {
      const resp = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ messages: [{ role: "user", content: fullPrompt }], model: "gpt-5.4" , useTools: true }),
      });
      if (resp.ok) {
      ...
      {open && (
        <motion.div className="fixed inset-0 z-[200] flex items-center justify-center p-4"
          ...
          <motion.div className="relative w-full max-h-[92dvh] overflow-hidden rounded-[18px] flex flex-col"
            style={{ width: "clamp(340px, 40vw, 560px)", backdropFilter: "blur(40px)", background: "#0a0a0a", border: "1px solid rgba(99,102,241,0.3)" }}
            initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0 }}>

            <div className="flex items-center justify-between px-4 pt-3 pb-[10px] border-b" style={{ borderColor: "rgba(99,102,241,0.15)" }}>
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: "rgba(99,102,241,0.12)", border: "1px solid rgba(99,102,241,0.35)" }}>
