    ...
    const promises = selectedModels.map(async (m) => {
      ...
      try {
        const res = await fetch("/api/chat", {
          ...
          body: JSON.stringify({ messages: [{ role: "user", content: q }], model: m.id, stream: true , useTools: true }),
          signal: abort.signal,
        });
      ...
      {open && (
        ...
        >
          <motion.div
            ...
            style={{ width: "clamp(340px, 40vw, 560px)", backdropFilter: "blur(40px)", background: "#080808", border: "1px solid rgba(99,102,241,0.25)", boxShadow: "0 0 60px rgba(99,102,241,0.1)" }}
          >
            <div className="flex items-center justify-between px-4 pt-3 pb-[10px] border-b" style={{ borderColor: "rgba(99,102,241,0.2)", background: "rgba(99,102,241,0.04)" }}>
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => { setResults([]); setInput(""); }} className="w-7 h-7 flex items-center justify-center rounded-lg text-gray-600 hover:text-indigo-400 transition-colors">
