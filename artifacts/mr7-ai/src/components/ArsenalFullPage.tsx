...
// Batch 16 — Uploaded Arsenal
import { AiderModal } from "./modals/AiderModal";
import { OpenDesignModal } from "./modals/OpenDesignModal";
import { OpenCodesignModal } from "./modals/OpenCodesignModal";
import { AIWebsiteClonerModal } from "./modals/AIWebsiteClonerModal";
import { StrixModal } from "./modals/StrixModal";
import { AwesomeHackingModal } from "./modals/AwesomeHackingModal";
import { OpenDesignProModal } from "./modals/OpenDesignProModal";
...
export function ArsenalFullPage({ moduleId, onBack }: Props) {
  ...
  const modal = (() => {
    switch (moduleId) {
      ...
      case "aider":           return <AiderModal open={open} onOpenChange={v => !v && onBack()} />;
      case "opendesign":      return <OpenDesignModal open={open} onOpenChange={v => !v && onBack()} />;
      case "opencodesign":    return <OpenCodesignModal open={open} onOpenChange={v => !v && onBack()} />;
      case "aiwebsitecloner": return <AIWebsiteClonerModal open={open} onOpenChange={v => !v && onBack()} />;
      case "strix":           return <StrixModal open={open} onOpenChange={v => !v && onBack()} />;
      case "awesomehacking":  return <AwesomeHackingModal open={open} onOpenChange={v => !v && onBack()} />;
      case "opendesignpro":   return <OpenDesignProModal open={open} onOpenChange={v => !v && onBack()} />;
      default:                return null;
    }
  })();
