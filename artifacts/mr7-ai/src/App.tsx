...
import { MinimizedArtifactsProvider } from "@/state/minimizedArtifacts";
import { MinimizedArtifactsBubbles } from "@/components/MinimizedArtifactsBubbles";
...
const FileManagerModal        = lazy(() => import("./components/modals/FileManagerModal").then(m=>({default:m.FileManagerModal})));
...
const InfrastructureDashboardPage = lazy(() => import("./pages/InfrastructureDashboard").then(m=>({default:m.InfrastructureDashboard})));
...
const MODAL_IDS = [
  ...
  'infraDashboard',
  // ── Ultra-Advanced Edition — Unified Enterprise System ─────────────────────
  'advancedDashboard',
  // ── Blueprint New Features ──────────────────────────────────────────────────
  'codeInterpreter',
  'customGpts',
...
function AppContent() {
  ...
  const [mobileNavTab, setMobileNavTab] = useState<"chat"|"arsenal"|"search"|"agent"|"more">("chat");
  ...
  // ── Arsenal Dynamic Tool-Calling Registry ──────────────────────────────────
  useEffect(() => { initializeToolsRegistry(); }, []);
  ...
  useEffect(() => {
    ...
    function onOpenFileManager() { open('fileManager'); }
    ...
    window.addEventListener("kali:open-file-manager", onOpenFileManager);
    return () => {
      ...
      window.removeEventListener("kali:open-file-manager", onOpenFileManager);
    };
  }, [open, dispatch]);
  ...
  const cachedProjects = state.cachedProjects ?? [];
  const activeProject = cachedProjects.find(p => p.id === activeProjectId) ?? null;

  function handleSelectProject(proj: Project | null) {
    dispatch({ type: "SET_ACTIVE_PROJECT", projectId: proj?.id ?? null });
    if (proj) {
      // auto-set project system instructions
      dispatch({ type: "SET_SETTINGS", patch: { customSystemPrompt: proj.system_instructions || "" } });
    }
  }
  ...
  return (
    ...
    <GlobalCommandPalette
      ...
      onOpenAnalytics={() => open('analyticsDashboard')}
    />
    <StudioLayout
      sidebarCollapsed={sidebarCollapsed}
      sidebarOpen={modals.sidebar}
      sidebar={<Sidebar
        ...
        onOpenAdvancedDashboard={() => open('advancedDashboard')}
        ...
        onOpenEcosystem={() => { window.location.href = "/ecosystem"; }}
      />}
      topBar={<TopBar
    ...
    >
      <div className="flex flex-col h-full overflow-hidden">
        ...
        {activeProject && (
          <div
            className="flex items-center gap-2 px-4 py-1.5 text-[11px] font-medium cursor-pointer hover:opacity-80 transition-opacity flex-shrink-0"
            style={{ background: `${activeProject.color}12`, borderBottom: `1px solid ${activeProject.color}25`, color: activeProject.color }}
          ...
          >
            ...
            {activeProject.system_instructions && (
              <span className="opacity-50 truncate flex-1 text-right font-mono" style={{ color: "rgba(255,255,255,0.4)" }}>
                · {activeProject.system_instructions.slice(0, 50)}
              </span>
            )}
          </div>
        )}
      ...
      <ProjectSwitcher
        ...
        onSelectProject={(proj) => {
          ...
          if (proj) {
            dispatch({ type: "CACHE_PROJECTS", projects: [...cachedProjects.filter(p => p.id !== proj.id), proj] });
          }
        }}
      />
      ...
      <Suspense fallback={null}>
        ...
        <FileManagerModal open={modals.fileManager} onOpenChange={(v) => mDispatch({type:'SET',id:'fileManager',value:v})} />
        ...
        {modals.infraDashboard && (
          <WindowChrome open={true} title="🔧 لوحة البنية التحتية — Admin" onClose={() => close('infraDashboard')}>
            <Suspense fallback={<div className="flex items-center justify-center h-64 text-zinc-500 text-sm">تحميل...</div>}>
              <InfrastructureDashboardPage onClose={() => close('infraDashboard')} />
            </Suspense>
          </WindowChrome>
        )}
        {/* ── Ultra-Advanced Unified Dashboard (Enterprise Edition) ─────── */}
        {modals.advancedDashboard && (
          <WindowChrome open={true} color="#e21227" title="🚀 UNIFIED SYSTEM v2.0 — Enterprise Dashboard" onClose={() => close('advancedDashboard')}>
            <AppErrorBoundary fallback={<ModalLoadErrorFallback title="Unified Dashboard" onClose={() => close('advancedDashboard')} />}>
              <Suspense fallback={<div className="flex items-center justify-center h-96 text-gray-500 text-sm">تحميل النظام الموحد...</div>}>
      ...
      <MobileBottomNav
        ...
        onTabChange={setMobileNavTab}
        onNewChat={() => { dispatch({ type: "NEW_CHAT" }); setMobileNavTab("chat"); }}
        onOpenArsenal={() => { open('arsenal'); setMobileNavTab("arsenal"); }}
        onOpenSearch={() => { open('search'); setMobileNavTab("search"); }}
        onOpenAgent={() => { open('agent'); setMobileNavTab("agent"); }}
        onOpenSettings={() => open('settings')}
        onOpenMemory={() => open('memory')}
        onOpenBookmarks={() => open('bookmarks')}
        onOpenDeepSearch={() => open('deepSearch')}
...
export default function App() {
  return (
    <QueryClientProvider client={queryClient}>
      <MinimizedArtifactsProvider>
        <StoreProvider>
          <TooltipProvider>
            <WindowManagerProvider>
              <AppContent />
              <Toaster />
              {/* Floating pills for minimized ArtifactPage overlays */}
