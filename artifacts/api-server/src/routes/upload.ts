    ...
    upload.single("file")(req, res, (err) => {
      if (err instanceof multer.MulterError) {
        if (err.code === "LIMIT_FILE_SIZE") {
          res.status(413).json({ error: `File too large. Maximum allowed size is ${MAX_UPLOAD_SIZE / 1024 / 1024}MB.` });
          return;
        }
        // Generic multer error — safe to say "invalid upload"
        res.status(400).json({ error: "Invalid file upload request." });
        return;
      }
      if (err) {
        // Custom fileFilter error: "File type not allowed: <mime>" — safe and informative
        const msg = err instanceof Error && err.message.startsWith("File type not allowed")
          ? err.message
