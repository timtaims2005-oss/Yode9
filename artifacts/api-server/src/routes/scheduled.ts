...
router.get("/scheduled/:id", async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  await ready();
  try {
...
router.put("/scheduled/:id", async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  await ready();
...
router.delete("/scheduled/:id", async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  await ready();
  try {
...
router.post("/scheduled/:id/run", async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  await ready();
  try {
    const result = await runTask(req.params["id"]);
    res.json({ ok: true, ...result });
  } catch (err) {
    res.status(500).json({ error: "Scheduled task failed. Please try again." });
  }
});

// ── POST /api/scheduled/:id/toggle ───────────────────────────────────────────
router.post("/scheduled/:id/toggle", async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  await ready();
  try {
    const { rows } = await pool.query(
