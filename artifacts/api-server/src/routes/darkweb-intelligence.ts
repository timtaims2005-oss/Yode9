...
router.get('/email/:email', async (req: Request<{ email: string }>, res: Response) => {
  try {
...
router.get('/ip/:ip', async (req: Request<{ ip: string }>, res: Response) => {
  try {
    const { ip } = req.params;
...
router.get('/domain/:domain', async (req: Request<{ domain: string }>, res: Response) => {
  try {
    ...
    const TIMEOUT = 6000;
    const headers = { 'User-Agent': 'mr7-osint/3.0' };

    // Gather real data in parallel from free public sources
    const [rdapRes, crtRes, dnsRes] = await Promise.allSettled([
      // RDAP WHOIS
      ...
      // DNS lookup via Cloudflare DoH (JSON API)
      fetch(`https://cloudflare-dns.com/dns-query?name=${encodeURIComponent(domain)}&type=ANY`, {
    ...
    const rdap: any  = rdapRes.status  === 'fulfilled' ? rdapRes.value  : null;
    const certs      = crtRes.status   === 'fulfilled' ? crtRes.value   : [];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const dns: any   = dnsRes.status   === 'fulfilled' ? dnsRes.value   : null;

    // Parse RDAP WHOIS data
    const whois: Record<string, any> = {};
    if (rdap) {
      whois.ldhName   = rdap.ldhName;
      whois.status    = rdap.status;
      whois.events    = (rdap.events ?? []).map((e: any) => ({
        action: e.eventAction, date: e.eventDate,
      }));
      whois.nameservers = (rdap.nameservers ?? []).map((ns: any) => ns.ldhName);
      const registrant = (rdap.entities ?? []).find((e: any) => (e.roles ?? []).includes('registrant'));
      if (registrant?.vcardArray?.[1]) {
        const vcard = registrant.vcardArray[1] as any[];
        const fn = vcard.find((f: any) => f[0] === 'fn');
        if (fn) whois.registrant = fn[3];
      }
    }

    // Parse certificate transparency
    const certificateList = Array.isArray(certs)
      ? [...new Set(
          (certs as any[]).slice(0, 20).map((c: any) => c.name_value?.split('\n')[0]).filter(Boolean)
        )].slice(0, 15)
      : [];

    // Parse DNS records
    const dnsRecords = dns?.Answer
      ? (dns.Answer as any[]).slice(0, 20).map((r: any) => ({
          name: r.name, type: r.type, data: r.data, ttl: r.TTL,
        }))
      : [];
    ...
    res.json({
      ...
      data: {
        domain,
        whois: Object.keys(whois).length > 0 ? whois : null,
        certificates: certificateList,
        dns: dnsRecords.length > 0 ? dnsRecords : null,
        sources: {
          rdap:  rdap  !== null,
          crtsh: certificateList.length > 0,
...
router.get('/blockchain/:address', async (req: Request<{ address: string }>, res: Response) => {
  try {
...
router.post('/blockchain/trace', async (req: Request, res: Response) => {
  try {
    ...
    if (!sourceAddress) {
      res.status(400).json({ success: false, error: 'sourceAddress is required' });
      return;
    }
...
router.get('/blockchain/tx/:hash', async (req: Request<{ hash: string }>, res: Response) => {
  try {
...
router.post('/blockchain/monitor', async (req: Request, res: Response) => {
  try {
    ...
    if (!addresses || addresses.length === 0) {
      res.status(400).json({ success: false, error: 'addresses are required' });
      return;
    }
...
router.get('/threat/ioc/:ioc', async (req: Request<{ ioc: string }>, res: Response) => {
  try {
...
router.get('/threat/actor/:actor', async (req: Request<{ actor: string }>, res: Response) => {
  try {
...
router.get('/graph/correlate/:type/:value', async (req: Request<{ type: string; value: string }>, res: Response) => {
  try {
...
router.post('/graph/query', async (req: Request, res: Response) => {
  try {
    ...
    if (!cypher) {
      res.status(400).json({ success: false, error: 'cypher query is required' });
      return;
    }
    ...
    if (dangerousPatterns.some(p => upperCypher.includes(p))) {
      res.status(403).json({ success: false, error: 'Destructive operations not allowed' });
      return;
    }
...
router.post('/ai/predict', async (req: Request, res: Response) => {
  try {
    ...
    if (!historicalData) {
      res.status(400).json({ success: false, error: 'historicalData is required' });
      return;
    }
...
router.get('/username/:username', async (req: Request<{ username: string }>, res: Response) => {
  try {
  ...
  try {
    ...
    res.json({
      ...
      data: {
        hash,
        type: detectedType,
        malicious: (rfResult.data as any)?.malicious || false,
        confidence: (rfResult.data as any)?.confidence || 0,
        malwareFamily: (rfResult.data as any)?.malwareFamily || null,
...
router.get('/domain/:domain/whois', async (req: Request<{ domain: string }>, res: Response) => {
  try {
    const { domain } = req.params;
...
router.get('/domain/:domain/dns', async (req: Request<{ domain: string }>, res: Response) => {
  try {
    const { domain } = req.params;
...
router.get('/domain/:domain/subdomains', async (req: Request<{ domain: string }>, res: Response) => {
  try {
    const { domain } = req.params;
    ...
    res.json({
      ...
      data: {
        domain,
        subdomains: (shodanResult.data as any)?.subdomains || [],
        total: (shodanResult.data as any)?.subdomains?.length || 0,
...
router.get('/domain/:domain/ssl', async (req: Request<{ domain: string }>, res: Response) => {
  try {
    const { domain } = req.params;
...
router.post('/sigint/search', async (req: Request, res: Response) => {
  try {
    ...
    if (!query) res.status(400).json({ success: false, error: 'query is required' });
      return;
...
router.post('/ioc/bulk', async (req: Request, res: Response) => {
  try {
    ...
    if (!iocs || !Array.isArray(iocs) || iocs.length === 0) {
      res.status(400).json({ success: false, error: 'iocs array is required' });
      return;
    }

    if (iocs.length > 100) {
      res.status(400).json({ success: false, error: 'Maximum 100 IOCs per request' });
      return;
    }
...
router.post('/person/search', async (req: Request, res: Response) => {
  try {
    ...
    if (!name && !email && !phone && !username) {
      res.status(400).json({ success: false, error: 'At least one search parameter is required' });
      return;
    }
...
router.post('/export', async (req: Request, res: Response) => {
  try {
    ...
    if (!data) res.status(400).json({ success: false, error: 'data is required' });
      return;
...
router.get('/evidence/:sessionId', async (req: Request<{ sessionId: string }>, res: Response) => {
  try {
    const { sessionId } = req.params;
    const evidence = evidenceStore.get(sessionId);
    if (!evidence) res.status(404).json({ success: false, error: 'Session not found' });
      return;
...
router.get('/cve/:cveId', async (req: Request<{ cveId: string }>, res: Response) => {
  try {
...
router.post('/timeline/add', async (req: Request, res: Response) => {
  try {
    ...
    if (!sessionId || !event) res.status(400).json({ success: false, error: 'sessionId and event required' });
      return;
...
router.get('/timeline/:sessionId', async (req: Request<{ sessionId: string }>, res: Response) => {
  try {
    const { sessionId } = req.params;
    const events = timelines.get(sessionId) || [];
