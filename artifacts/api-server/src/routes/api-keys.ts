...
import {
  requireUnifiedAuth,
  authAuditLog,
} from "../middlewares/unifiedAuthMiddleware";
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    const userId = req.unifiedAuth.userId;
    try {
      const { rows } = await pool.query(
        `SELECT id, key_prefix, name, permissions, rate_limit_per_min,
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      const { rows } = await pool.query(
        ...
        [userId, hash, prefix, name || "My API Key", JSON.stringify(permissions ?? ["chat"]), expiresAt],
      );

      authAuditLog(req, "api_key.created", { keyName: name });
...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      authAuditLog(req, "api_key.revoked", { keyId: req.params.id });
      res.json({ ok: true });
    } catch {
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      const { rows } = await pool.query(
        ...
        [userId, url, JSON.stringify(events ?? ["task.done"]), secret],
      );
      authAuditLog(req, "webhook.created", { webhookUrl: url });
