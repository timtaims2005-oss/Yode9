 ...
 * @deprecated
 * ─────────────────────────────────────────────────────────────────────────────
 * DEPRECATED: This file (db.ts) uses raw SQL for schema management and should
 * NOT be extended. All new tables and queries must be defined in lib/db/src/schema/
 * using Drizzle ORM.
 *
 * Migration status:
 *  - Table creation: still managed here via ensureAuthTables() (raw SQL IF NOT EXISTS)
 *  - CRUD helpers: still exported from here (getUserByEmail, createUser, etc.)
 *  - All new features: must use lib/db (Drizzle) exclusively
 *
 * Removal plan:
 *  1. Migrate remaining CRUD helpers to Drizzle repositories under lib/db
 *  2. Replace ensureAuthTables() with Drizzle migrate() / push()
 *  3. Delete this file
 * ─────────────────────────────────────────────────────────────────────────────
 */
import { Pool } from "pg";
...
export async function ensureAuthTables() {
  try {
    ...
    // API keys (developer-issued mr7_* keys — full schema)
    await pool.query(`
      CREATE TABLE IF NOT EXISTS api_keys (
        ...
        key_prefix VARCHAR,
        name VARCHAR NOT NULL DEFAULT 'My API Key',
        label VARCHAR,
    ...
    // Safe migrations: add new columns to existing api_keys table
    await pool.query(`ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS user_id VARCHAR REFERENCES users(id) ON DELETE CASCADE`).catch(() => {});
    await pool.query(`ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS key_prefix VARCHAR`).catch(() => {});
    await pool.query(`ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS name VARCHAR NOT NULL DEFAULT 'My API Key'`).catch(() => {});
    await pool.query(`ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS permissions JSONB DEFAULT '["chat"]'::jsonb`).catch(() => {});
    await pool.query(`ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS rate_limit_per_min INTEGER NOT NULL DEFAULT 60`).catch(() => {});
    await pool.query(`ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS last_used_at TIMESTAMP WITH TIME ZONE`).catch(() => {});
    await pool.query(`ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS expires_at TIMESTAMP WITH TIME ZONE`).catch(() => {});
    await pool.query(`ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT true`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_api_keys_user_id ON api_keys (user_id)`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_api_keys_hash ON api_keys (key_hash)`).catch(() => {});

    // Webhook endpoints (developer webhooks)
    await pool.query(`
      CREATE TABLE IF NOT EXISTS webhook_endpoints (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        user_id VARCHAR REFERENCES users(id) ON DELETE CASCADE,
        url TEXT NOT NULL,
        events JSONB NOT NULL DEFAULT '["task.done"]'::jsonb,
    ...
    `).catch((err) => logger.warn({ err }, "webhook_endpoints table may already exist"));
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_webhook_endpoints_user_id ON webhook_endpoints (user_id)`).catch(() => {});
    ...
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_conversations_created_at ON conversations (created_at DESC)`).catch(() => {});
    ...
    `).catch(() => {});
    // ── Indexes: messages ─────────────────────────────────────────────────────
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_messages_conversation_id ON messages (conversation_id)`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_messages_created_at      ON messages (created_at DESC)`).catch(() => {});
    ...
    // ── AI User Memories with pgvector embeddings ──────────────────────────
    // Stores agent memories + 3072-dim embeddings (Google gemini-embedding-001)
    ...
    `).catch((err) => logger.warn({ err }, "ai_user_memories table may already exist"));
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_ai_user_memories_user_id ON ai_user_memories (user_id)`).catch(() => {});
    // Note: pgvector ivfflat/hnsw indexes are limited to 2000 dims; for 3072-dim
    // embeddings we rely on exact cosine search (sequential scan per user_id).

    // ── Sensitive tool audit log (Task 1 — Guardrails) ───────────────────────
    await pool.query(`
      CREATE TABLE IF NOT EXISTS sensitive_tool_audit_log (
        id SERIAL PRIMARY KEY,
        device_id VARCHAR,
        ip VARCHAR,
        tool_name VARCHAR NOT NULL,
        args_summary TEXT,
        verdict VARCHAR NOT NULL CHECK (verdict IN ('allowed','blocked')),
        block_reason TEXT,
        created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
      )
    `).catch((err) => logger.warn({ err }, "sensitive_tool_audit_log table may already exist"));
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_staudit_tool ON sensitive_tool_audit_log (tool_name, created_at)`).catch(() => {});

    // ── Project file version history (Task 2 — Version History) ──────────────
    await pool.query(`
    ...
    `).catch((err) => logger.warn({ err }, "project_file_versions table may already exist"));
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_pfv_lookup ON project_file_versions (project_id, filename, version_number DESC)`).catch(() => {});
