...
// Subscriber client map for pub/sub (keyed by channel)
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const _subClients = new Map<string, any>();
  ...
  try {
    ...
    const backend: CacheBackend = {
      ...
      subscribe: async (ch, handler) => {
        ...
        // Store sub client keyed by channel so we can unsubscribe later
        _subClients.set(ch, sub);
      },
      unsubscribe: async (ch) => {
        const sub = _subClients.get(ch);
        if (sub) {
          await sub.unsubscribe(ch).catch(() => {});
          await sub.quit().catch(() => {});
          _subClients.delete(ch);
        }
      },
