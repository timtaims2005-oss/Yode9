...
/**
 * Sentry Express performance middleware.
 * Wraps each request in a Sentry transaction for route-level timing.
 * Call BEFORE route handlers; call sentryErrorHandler AFTER routes.
 *
 * Usage in app.ts:
 *   const { sentryRequestHandler, sentryTracingHandler, sentryErrorHandler } = await getSentryHandlers();
 *   app.use(sentryRequestHandler());
 *   app.use(sentryTracingHandler());
 *   // ... routes ...
 *   app.use(sentryErrorHandler());
 */
export async function getSentryHandlers(): Promise<{
  sentryRequestHandler: () => import("express").RequestHandler;
  sentryTracingHandler: () => import("express").RequestHandler;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  sentryErrorHandler: () => import("express").ErrorRequestHandler<any, any, any, any>;
}> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const noop = () => (_req: unknown, _res: unknown, next: (...args: any[]) => void) => next();

  if (!sentryInitialized) {
    return {
      sentryRequestHandler: noop as () => import("express").RequestHandler,
      sentryTracingHandler:  noop as () => import("express").RequestHandler,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      sentryErrorHandler:    noop as unknown as () => import("express").ErrorRequestHandler<any, any, any, any>,
    };
  }

  try {
    const Sentry = await import("@sentry/node");
    // Sentry v8+: expressRequestHandler / expressTracingHandler were removed.
    // Request tracking and tracing are now handled automatically via expressIntegration()
    // configured in Sentry.init(). Only the error handler needs to be wired manually.
    const identityMiddleware: import("express").RequestHandler = (_req, _res, next) => next();
    return {
      sentryRequestHandler: () => identityMiddleware,
      sentryTracingHandler: () => identityMiddleware,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      sentryErrorHandler:   () => Sentry.expressErrorHandler() as unknown as import("express").ErrorRequestHandler<any, any, any, any>,
    };
  } catch {
    return {
      sentryRequestHandler: noop as () => import("express").RequestHandler,
