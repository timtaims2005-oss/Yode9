...
export function validateEnv(): void {
  ...
  // Warn if LOCAL_ENGINE_API_KEY is missing
  if (!process.env.LOCAL_ENGINE_API_KEY) {
    logger.warn(
      "LOCAL_ENGINE_API_KEY is not set — local model engine endpoints (Ollama, local-engines, local-proxy) are publicly accessible. Set this secret to restrict access.",
    );
  }
