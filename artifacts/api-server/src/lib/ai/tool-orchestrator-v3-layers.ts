...
import { chatWithTools, ToolContext, TOOL_CAPABLE_MODELS, ChatWithToolsResult } from "./tool-orchestrator-v2.js";
...
// --- 3) نظام الـ fallback الذكي ---
// ملاحظة: chatWithTools تحمل الـ fallback داخلها (Groq → OpenRouter → Gemini → Cloudflare).
// لا داعي لتكرار الاستدعاء هنا — استدعاء واحد يكفي.
export const FALLBACK_CHAIN = [
  "llama-3.3-70b-versatile",         // Groq (الأول)
  "meta-llama/llama-3.3-70b-instruct:free", // OpenRouter (الثاني)
  "gemini-2.5-flash",                // Gemini (الثالث)
  "@cf/meta/llama-3.3-70b-instruct-fp8-fast", // Cloudflare (الأخير)
];
...
export async function chatWithToolsRobust(
  ...
  // chatWithTools تتعامل مع الـ fallback داخلياً — استدعاء واحد يكفي
  const result = await chatWithTools(ctx, preferredModel, messages, options);
  const finalResult = { ...result, modelUsed: result.providerUsed ?? preferredModel, cached: false };
